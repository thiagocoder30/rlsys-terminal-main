...
test(
  'scripted paper runtime preserves ledger command preloads',
  () => {
    const {
      dir,
      result,
    ...
    assert.match(
      output,
      /Ledger recorded: WIN 10/,
    );

    assert.match(
      output,
      /Ledger recorded: LOSS 3/,
    );
...
test(
  'scripted paper runtime preserves operator discipline preloads',
  () => {
    const {
      dir,
      result,
...
test(
  'scripted paper runtime exits deterministically and tolerates incompatible snapshot notice',
  () => {
    const dir =
      fs.mkdtempSync(
        path.join(
          os.tmpdir(),
          'rlsys-paper-preload-bad-snapshot-',
        ),
      );
