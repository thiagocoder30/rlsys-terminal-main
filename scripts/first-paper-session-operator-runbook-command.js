...
async function main() {
  ...
  if (input.format === 'json') {
    const result = await command.compose(payload, generatedAtEpochMs);
  ...
  const result = await command.textReport(payload, generatedAtEpochMs);
...
main().catch((error) => {
  printJson({
    ok: false,
    error: {
      code: 'FIRST_PAPER_SESSION_OPERATOR_RUNBOOK_COMMAND_CLI_ERROR',
