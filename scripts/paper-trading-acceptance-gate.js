...
async function main() {
  ...
  if (input.format === 'json') {
    const result = await gate.evaluate(payload, generatedAtEpochMs);
  ...
  const result = await gate.textReport(payload, generatedAtEpochMs);
...
main().catch((error) => {
  printJson({
    ok: false,
    error: {
      code: 'PAPER_TRADING_ACCEPTANCE_GATE_CLI_ERROR',
