...
echo "======================================"
echo " RL.SYS CORE - SPRINT 613"
echo " ELITE UI (NO NUMPAD) & TACTICAL ORACLE"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR/pwa-terminal"

# 1. ARQUIVO INTEGRAL DO CÉREBRO: src/core/useTacticalEngine.ts
...
export function useTacticalEngine(initialBankroll = 55.00) {
    ...
    const [actionLogs, setActionLogs] = useState<string[]>(() => loadCache('rl_logs', ['[SISTEMA] Motor Online. Oráculo Tático Ativado.']));
    ...
    const [auditReason, setAuditReason] = useState<string>(""); // Novo: Razão da Máquina
    const [oracleMessage, setOracleMessage] = useState<string>("Sincronize a fita para ativar o Oráculo."); // Novo: O Oráculo
    ...
    // Motor do Oráculo (Análise Comportamental)
    const updateOracle = (currentVix: number, currentTimeline: number[]) => {
        if (currentTimeline.length < 5) {
            setOracleMessage("Volume de dados insuficiente. Continue observando.");
            return;
        }
        
        let msg = "";
        const recent = currentTimeline.slice(-10);
        const zeroCount = recent.filter(n => n === 0).length;
        const uniqueSet = new Set(recent);
        
        if (zeroCount >= 2) msg = "ALERTA: Alta densidade de Zeros (Green) detectada recentemente. Mesa hostil.";
        else if (currentVix > 90) msg = "MESA EM CAOS. Alta entropia e ausência de padrões. O algoritmo exige pesos massivos (> 1.50) para atirar.";
        else if (currentVix < 30) msg = "MESA FRIA. Extrema repetição de números. Padrões viciados identificados. Reatores sensíveis.";
        else if (uniqueSet.size < 6) msg = "Anomalia: A roleta travou em um setor específico. Acompanhe a tendência sem forçar cruzamentos.";
        else msg = "Mesa estável. Entropia em níveis nominais. Siga rigorosamente as ordens de Engage.";
    ...
    useEffect(() => {
        ...
        if (timeline.length > 3) {
            ...
            currentVix = Math.max(10.0, Math.min(99.9, baseEntropy + (Math.random() * 4)));
            setVix(currentVix);
        } else {
            setVix(0.0);
        }

        updateOracle(currentVix, timeline);

        const requiredWeight = currentVix > 85 ? 1.50 : 1.25;
        ...
        if (bestStrat) {
            ...
            if (totalStake <= safeLimit && baseUnits > 0) {
                ...
                setAuditReason(`Gatilho Superado. O Peso da estratégia (${highestWeight.toFixed(2)}) é maior que a exigência da mesa (${requiredWeight.toFixed(2)}). PnL Shadow positivo.`);
            } else { 
                setActiveStrategy(null); setActiveStake(0); setActiveDesc(""); 
                setAuditReason(`Alocação negada. Stake ultrapassa teto de segurança (5% da banca) ou fichas insuficientes.`);
            }
        } else { 
            setActiveStrategy(null); setActiveStake(0); setActiveDesc(""); 
            setAuditReason(`Nenhuma estratégia atingiu o peso exigido de ${requiredWeight.toFixed(2)} ou estão bloqueadas no Shadow PnL (prejuízo virtual).`);
        }
...
# 2. ARQUIVO DA INTERFACE LIMPA (SEM NUMPAD, COM ORÁCULO E XAI)
cat > src/App.tsx <<'EOF'
...
export default function App() {
  ...
  const handleFastInput = (e: React.FormEvent | React.MouseEvent, isSkip: boolean = false) => {
      ...
      if (!isNaN(num) && num >= 0 && num <= 36) {
          ...
          setFastInput('');
          // Re-foca no input para digitação em massa
  ...
  return (
    <div className="h-[100dvh] w-full bg-obsidian flex flex-col overflow-hidden relative font-mono text-gray-300 select-none">
      ...
      {/* ZONA C: Painel de Combate Puro (Sem Numpad Visual) */}
      <div className="flex-1 flex flex-col overflow-y-auto no-scrollbar p-4 pb-28 z-0">
        ...
        {/* CLI LIVE LOG TERMINAL (Flex 1 para empurrar o input pro fundo) */}
        ...
        {/* CONSOLE NUMÉRICO GIGANTE (Substitui o Numpad Visual) */}
      ...
      {/* MODALS OVERLAYS (Terminal, Stats, Config permanecem inalterados) */}
      {activeModal && (
...
echo "[RL.SYS] Compilação Concluída. Numpad removido, UI purificada."
echo "======================================"
