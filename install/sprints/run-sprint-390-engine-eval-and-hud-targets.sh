...
export class LivePaperOrchestrator {
    ...
    // FIX SPRINT 390: Coordenada do Alvo na Tela
    ...
    private generateNextTrade(): void {
        ...
        this.customTargetInfo = null; // Reseta a coordenada
        ...
        if (!this.activeStrategyId) {
            ...
            if (bestStrat && maxScore >= (timeline.length * 0.40)) { 
                ...
                // FIX SPRINT 390: Injetar a coordenada visual no HUD
                if (bestStrat === 'CROSS_GRID_HEDGE') {
                    const lastNum = operationalHistory[operationalHistory.length - 1];
                    if (lastNum >= 1 && lastNum <= 12) this.customTargetInfo = '[COLUNAS 2 e 3]';
                    else if (lastNum >= 13 && lastNum <= 24) this.customTargetInfo = '[COLUNAS 1 e 3]';
                    else if (lastNum >= 25 && lastNum <= 36) this.customTargetInfo = '[COLUNAS 1 e 2]';
                }
            }
        }
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            // FIX SPRINT 390: Avaliação direta pelo dicionário para evitar TypeError e coletar o Anchor (Número Anterior)
            if (this.activeStrategyId) { 
    ...
    private renderTerminalHud(): void {
        ...
        if (toxicLockActive) {
            ...
            // FIX SPRINT 390: Exibição visual do Alvo
