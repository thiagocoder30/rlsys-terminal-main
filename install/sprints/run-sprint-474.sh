...
echo "======================================"
echo " RL.SYS CORE - SPRINT 474"
echo " CORREÇÃO DA DIRETRIZ DE PULO (SKIP)"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (!isNaN(num) && num >= 0 && num <= 36) {
                ...
                if (isSkipped) {
                    this.lastActionTakenText = `\x1b[33mGiro [${num}] sincronizado passivamente (PULADO).\x1b[0m`;
    ...
    private generateNextTrade(isSkipped: boolean = false) {
        ...
        // Cálculo estrito da Entropia (VIX)
        if (history.length > 3) {
        ...
        // Bloqueio de cálculos se a rodada for do tipo SKIP
        if (isSkipped) {
            this.xaiQualification = 'NÃO';
            this.xaiMoment = 'PULADO';
            this.xaiReason = 'Sincronização passiva. Motor de apostas ignorou a rodada.';
            ...
            // Escolhe estratégia ativa se houver reatores ligados
            const activeStrats = Object.keys(this.shadowWeights).filter(id => !this.disabledStrategies.has(id));
...
echo "[RL.SYS] Sprint 474 finalizada com sucesso."
