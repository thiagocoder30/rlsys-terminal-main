...
echo "======================================"
echo " RL.SYS CORE - SPRINT 345"
echo " FUSION HEATMAP SHADOW INTEGRATION"
echo "======================================"
...
BACKUP_FILE="$ENGINE_FILE.bak.sprint345"

LOG_DIR="/sdcard/Download/rlsys/logs/sprint-345"
mkdir -p "$LOG_DIR"

MAIN_LOG="$LOG_DIR/sprint-345.log"
...
echo "[2/6] Aplicando Observabilidade e Shadow Computation dupla..."
...
if ! npm test > "$TEST_LOG" 2>&1; then
  ...
  mv "$BACKUP_FILE" "$ENGINE_FILE"
  grep -i "fail" "$TEST_LOG" | head -n 5 || tail -n 10 "$TEST_LOG"
  exit 1
fi
...
git add "$ENGINE_FILE"
git commit -m "sprint(345): implement observational shadow integration for FusionHeatmapEngine

- introduce FusionHeatmapIntegrationEngine as a parallel observational layer
- map advanced WheelNumberHeat metrics to enable direct comparison with legacy outputs
- capture deep contextual metrics (fusion pressure, recency pressure, dispersion)
- strictly preserve legacy computeHeatmap as the functional source of truth
- maintain zero-impact on final decision, confidence, or risk output
- enforce CI/CD pipeline integrity before commit" > /dev/null

echo "[5/6] Removendo artefatos temporários..."
rm -f "$ENGINE_FILE.bak"*

echo "[6/6] Finalizando..."
echo "======================================"
echo -e "\033[1;32m SPRINT 345 FINALIZADA COM SUCESSO \033[0m"
echo " SHADOW LAYER 1: TRIPLICAÇÃO (PARITY GATE)"
echo " SHADOW LAYER 2: FUSION HEATMAP (TELEMETRY)"
echo " CI/CD: PASSED"
echo "======================================"
