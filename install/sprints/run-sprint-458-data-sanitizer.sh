...
echo "======================================"
echo " RL.SYS CORE - SPRINT 458"
echo " DATA SANITIZER ENGINE"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/2] Injetando o Motor Regex de Limpeza no Orquestrador..."
...
export class LivePaperOrchestrator {
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            // [NOVO] MOTOR DE LIMPEZA DE DADOS (DATA SANITIZER)
            if (cmd.startsWith('clean ')) {
                const targetFile = line.replace(/clean\s+/i, '').trim();
                console.clear();
                console.log('======================================================');
                console.log(' 🧹 MOTOR DE LIMPEZA E EXTRAÇÃO DE DADOS (SANITIZER)');
                console.log('======================================================');
                
                try {
                    const rawContent = fs.readFileSync(path.resolve(process.cwd(), targetFile), 'utf8');
                    // Expressão Regular pura: varre o texto caótico procurando exatamente números de 0 a 36 isolados
                    const regex = /\b([0-9]|[1-2][0-9]|3[0-6])\b/g;
                    const matches = rawContent.match(regex);
                    
                    if (matches && matches.length > 0) {
                        const cleanSequence = matches.join(',');
                        const outPath = path.join(process.cwd(), 'data', 'fita_limpa.txt');
                        fs.writeFileSync(outPath, cleanSequence);
                        console.log(`\x1b[32m[SUCESSO] ${matches.length} números válidos de roleta extraídos!\x1b[0m`);
                        console.log(` Arquivo limpo salvo em: \x1b[36m${outPath}\x1b[0m`);
                        console.log(` Agora você pode rodar: \x1b[33mbacktest data/fita_limpa.txt\x1b[0m`);
                    } else {
                        console.log('\x1b[31m[ERRO] Nenhum número válido de roleta encontrado no texto.\x1b[0m');
                    }
                } catch (e) {
                    console.log(`\x1b[31m[ERRO] Não foi possível ler o arquivo: ${targetFile}\x1b[0m`);
                    console.log(` Certifique-se de que o arquivo existe (ex: nano data/sujo.txt e cole o texto lá).`);
                }
            ...
            if (cmd === 'help') {
                ...
                console.log(' clean <arquivo>      : Extrai giros válidos de texto sujo (Web)');
...
echo "======================================"
echo -e "\033[1;32m SPRINT 458 INSTALADA COM SUCESSO \033[0m"
echo " STATUS: DATA SANITIZER ATIVADO"
echo "======================================"
