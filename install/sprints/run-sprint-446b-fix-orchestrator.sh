...
echo "======================================"
echo " RL.SYS CORE - SPRINT 446-B"
echo " ORCHESTRATOR COMMAND HOTFIX"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/2] A reescrever o Orchestrator com a ordem correta..."
...
import { RuntimeEventBus, PooledSignal } from '../../domain/memory/SignalObjectPool';
...
export class LivePaperOrchestrator {
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            const cmd = line.trim().toLowerCase(); // DECLARAÇÃO PRIMEIRO
