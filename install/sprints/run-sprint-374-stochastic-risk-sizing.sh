...
echo "======================================"
echo " RL.SYS CORE - SPRINT 374"
echo " STOCHASTIC RISK & SIZING ENGINE"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/4] Projetando Motores de Domínio (Sizing & Trailing Stop)..."
...
cat > src/domain/risk/TrailingStopGuard.ts <<'EOF'
...
echo "[2/4] Executando Validação Interna em Sandbox (TDD)..."
mkdir -p tests
...
# Roda o teste isolado para confirmar a matemática
npx ts-node tests/sprint-374-risk.test.ts || exit 1

echo "[3/4] Injetando Motores Dinâmicos na Camada de Apresentação..."
...
echo "[4/4] Registrando os Escudos Dinâmicos na árvore Git..."
git add src/domain/risk/PositionSizingEngine.ts src/domain/risk/TrailingStopGuard.ts tests/sprint-374-risk.test.ts src/presentation/cli/LivePaperOrchestrator.ts
git commit -m "feat(risk): implement Fractional Kelly position sizing, Provider floor limits, and Trailing Stop loss protection (Sprint 374)" > /dev/null

echo "======================================"
echo -e "\033[1;32m SPRINT 374 INSTALADA COM SUCESSO \033[0m"
echo " STATUS: FRACTIONAL KELLY E TRAILING STOP ATIVOS"
echo "======================================"
