...
echo "======================================"
echo " RL.SYS CORE - SPRINT 495"
echo " EXPANSÃO DE ARSENAL (DÚZIAS, P2, POTINHO, 1 e 21)"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    private renderTerminalHud(): void {
        ...
        console.log(' RL.SYS CORE - TACTICAL ORCHESTRATOR [V5.13b]');
        console.log('\x1b[36m======================================================\x1b[0m');
    ...
    private resolveFinancials(drawnNumber: number) {
        ...
        } else if (strat.startsWith('SECTOR_') || strat.startsWith('ZONE_') || strat === 'FUSION_REDUZIDA' || strat === 'PATTERN_FRANCESA') {
    ...
    private calculateSizing(strategyId: string, minChip: number, weight: number): { total: number, desc: string, multiplier: number, isSafe: boolean, cost: number, safeLimit: number } {
        ...
        let baseUnits = 0;
        if (strategyId.startsWith('CROSS_GRID') || strategyId.startsWith('CROSS_DOZEN')) baseUnits = 2; 
        else if (strategyId.startsWith('SECTOR_') || strategyId.startsWith('ZONE_') || strategyId === 'FUSION_REDUZIDA' || strategyId === 'PATTERN_FRANCESA') baseUnits = this.STRATEGY_ZONES[strategyId].length; 
        ...
        if (strategyId.startsWith('SECTOR_') || strategyId.startsWith('ZONE_') || strategyId === 'FUSION_REDUZIDA' || strategyId === 'PATTERN_FRANCESA') {
                ...
                if (bestStrat !== 'Nenhuma') {
                    ...
                    for (const num of historyToTest) {
                        ...
                        if (bestStrat.startsWith('CROSS_GRID') || bestStrat.startsWith('CROSS_DOZEN')) {
                            const cost = 2 * minChip;
                            pnl = isWin ? (3 * minChip) - cost : -cost;
                        } else if (bestStrat.startsWith('SECTOR_') || bestStrat.startsWith('ZONE_') || bestStrat === 'FUSION_REDUZIDA' || bestStrat === 'PATTERN_FRANCESA') {
                            const cost = zone.size * minChip;
                            pnl = isWin ? (36 * minChip) - cost : -cost;
            ...
            if (this.activeStrategyId !== 'Nenhuma') {
                ...
                if (safeWeight < requiredWeight) {
                    ...
                    if (!sizing.isSafe) {
                        ...
                        let winProfit = 0;
                        if (this.activeStrategyId.startsWith('CROSS_GRID') || this.activeStrategyId.startsWith('CROSS_DOZEN')) winProfit = (3 * minChip * sizing.multiplier) - sizing.total;
                        else if (this.activeStrategyId.startsWith('SECTOR_') || this.activeStrategyId.startsWith('ZONE_') || this.activeStrategyId === 'FUSION_REDUZIDA' || this.activeStrategyId === 'PATTERN_FRANCESA') winProfit = (36 * minChip * sizing.multiplier) - sizing.total;
...
echo "[RL.SYS] Compilando novo Arsenal Tático (V5.13b)..."
npx tsc || npm run build || true
echo "[RL.SYS] Sprint 495 Concluída."
