...
echo "======================================"
echo " RL.SYS CORE - SPRINT 466-B"
echo " FULL HFT CORE (NO SHORTCUTS)"
echo "======================================"
...
export class LivePaperOrchestrator {
        ...
        this.rl.on('line', (line) => {
            ...
            if (cmd.startsWith('backtest ')) {
                ...
                backtestSequence.forEach(n => {
                    ...
                    let hc1=0,hc2=0,hc3=0; let hd1=0,hd2=0,hd3=0;
                    simHistory.slice(-20).forEach(hn => {
                        if(hn!==0){
                            if(hn%3===1)hc1++; else if(hn%3===2)hc2++; else hc3++;
                            if(hn<=12)hd1++; else if(hn<=24)hd2++; else hd3++;
                        }
                    });
                    ...
                    if(bStrat) {
                        ...
                        else if (bStrat === 'CROSS_GRID_HEDGE') {
                            const cNum = (n % 3 === 0) ? 3 : (n % 3);
                            isWin = hCols.includes(cNum);
                        }
            ...
            if (cmd === 'weights') {
                ...
                Object.entries(this.shadowWeights).forEach(([id, w]) => {
                    ...
                    console.log(` Estratégia: ${id.padEnd(20)} | PnL Monetário: ${pnlColor}${pnl > 0 ? '+' : ''}${pnl.toFixed(2)}\x1b[0m | Peso RL: ${Number(w).toFixed(2)} | ${st}`);
                });
                console.log('======================================================');
                console.log('Pressione ENTER para retornar ao HUD...');
                this.inputMode = 'VIEW_ONLY';
    ...
    private processRealSettlement(num: number, isSkipped: boolean): void {
        ...
        if (this.lastQualification === 'SIM' && this.lastRecommendedStrategy && this.lastRecommendedStake > 0) {
            ...
            if (num === 0 && id !== 'SECTOR_VOISINS') { isWin = false; } 
                ...
                isWin = this.SECTOR_FUSION.has(num); profitMultiplier = 0.89; 
            }
    ...
    private processShadowTrading(num: number): void {
        ...
        for (const id of Object.keys(this.shadowWeights)) {
            ...
            if (num === 0 && id !== 'SECTOR_VOISINS') isWin = false;
                ...
                isWin = this.SECTOR_FUSION.has(num); profitMulti = 0.89;
            }
    ...
    private checkCircuitBreakers(): void {
        ...
        if (this.cooldownGuard.currentBankroll >= this.takeProfitM3) {
            ...
            this.broadcastLiveTelemetry(); 
            this.generateInstitutionalReport('TAKE_PROFIT');
            process.exit(0);
        }
        if (this.cooldownGuard.currentBankroll <= this.hardStopLoss) {
            ...
            this.broadcastLiveTelemetry(); 
            this.generateInstitutionalReport('STOP_LOSS');
            process.exit(0);
        }
    }
    ...
    private getPlacementInstruction(strategy: string | null, totalStake: number, hedge: number): string {
        ...
        if (strategy === 'TRIPLICACAO_RED') baseInst = `${fmt(totalStake)} no \x1b[31mVERMELHO\x1b[0m`;
            ...
            baseInst = `${fmt(totalStake)} na Pista: \x1b[36mFUSION (Setor 23 / 19 nums)\x1b[0m`;
        }
    ...
    private renderTerminalHud(): void {
        console.clear();
        console.log('======================================================');
        console.log(' 🛡️  RL.SYS CORE - TACTICAL ORCHESTRATOR [V5.8b]');
        console.log('======================================================');
...
echo "[2/2] Compilando motor com Comandos Totais..."
npx tsc

echo "======================================"
echo -e "\033[1;32m NÚCLEO RESTAURADO E SELADO \033[0m"
echo " STATUS: TODOS OS COMANDOS CLI 100% FUNCIONAIS"
echo "======================================"
