...
echo "======================================"
echo " RL.SYS CORE - SPRINT 494"
echo " ARQUITETURA CONDICIONAL & ESTRATÉGIA FRANCESA"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    private renderTerminalHud(): void {
        ...
        console.log(' RL.SYS CORE - TACTICAL ORCHESTRATOR [V5.12b]');
        console.log('\x1b[36m======================================================\x1b[0m');
    ...
    private resolveFinancials(drawnNumber: number) {
        ...
        } else if (strat.startsWith('SECTOR_') || strat === 'FUSION_REDUZIDA' || strat === 'PATTERN_FRANCESA') {
    ...
    private evaluateShadowTrading(drawnNumber: number) {
        ...
        for (const strat of Object.keys(this.STRATEGY_ZONES)) {
            ...
            // Tratamento especial para Estratégias Condicionais (Gatilhos)
            if (strat === 'PATTERN_FRANCESA' && lastNum !== 28 && lastNum !== 29) {
                continue; // Pula PnL fantasma se o gatilho não foi acionado
            }
    ...
    private calculateSizing(strategyId: string, minChip: number, weight: number): { total: number, desc: string, multiplier: number, isSafe: boolean, cost: number, safeLimit: number } {
        ...
        else if (strategyId.startsWith('SECTOR_') || strategyId === 'FUSION_REDUZIDA' || strategyId === 'PATTERN_FRANCESA') baseUnits = this.STRATEGY_ZONES[strategyId].length; 
        ...
        if (strategyId.startsWith('SECTOR_') || strategyId === 'FUSION_REDUZIDA' || strategyId === 'PATTERN_FRANCESA') {
        ...
        if (strategyId.startsWith('TRIPLICACAO_')) {
            let targetName = strategyId.replace('TRIPLICACAO_', '');
            return { total: totalCost, desc: `\x1b[32mAposta Externa Única em ${targetName} (R$ ${totalCost.toFixed(2)})\x1b[0m`, multiplier, isSafe: true, cost: baseCost, safeLimit };
        }
        return { total: 0, desc: 'Aguardando validação.', multiplier: 1, isSafe: true, cost: 0, safeLimit };
    }
            ...
            if (lowerCmd.startsWith('backtest')) {
                ...
                for (const num of historyToTest) {
                    for (const strat of Object.keys(this.STRATEGY_ZONES)) {
                        ...
                        // Respeito ao gatilho condicional no Backtest
                ...
                if (bestStrat !== 'Nenhuma') {
                    ...
                    for (const num of historyToTest) {
                        ...
                        if (bestStrat.startsWith('CROSS_GRID')) {
                            const cost = 2 * minChip;
                            pnl = isWin ? (3 * minChip) - cost : -cost;
                        } else if (bestStrat.startsWith('SECTOR_') || bestStrat === 'FUSION_REDUZIDA' || bestStrat === 'PATTERN_FRANCESA') {
                            const cost = zone.size * minChip;
                            pnl = isWin ? (36 * minChip) - cost : -cost;
    ...
    private generateNextTrade() {
        ...
        // Identificação do último número para ativar Gatilhos Markovianos (PATTERN)
        const lastNum = history.length > 0 ? history[history.length - 1] : -1;
        ...
        if (this.currentVixPercent > this.dynamicVixTolerance) {
            const activeStrats = Object.keys(this.STRATEGY_ZONES)
                .filter(id => !this.disabledStrategies.has(id))
                .filter(id => {
                    // O filtro só permite a PATTERN_FRANCESA se o 28 ou 29 acabou de cair
            ...
            if (this.activeStrategyId !== 'Nenhuma') {
                ...
                if (safeWeight < requiredWeight) {
                    ...
                    if (!sizing.isSafe) {
                        ...
                        let winProfit = 0;
                        if (this.activeStrategyId.startsWith('CROSS_GRID')) winProfit = (3 * minChip * sizing.multiplier) - sizing.total;
                        else if (this.activeStrategyId.startsWith('SECTOR_') || this.activeStrategyId === 'FUSION_REDUZIDA' || this.activeStrategyId === 'PATTERN_FRANCESA') winProfit = (36 * minChip * sizing.multiplier) - sizing.total;
                        else if (this.activeStrategyId.startsWith('TRIPLICACAO_')) winProfit = (2 * minChip * sizing.multiplier) - sizing.total;
...
echo "[RL.SYS] Compilando a Camada Condicional de Padrões..."
npx tsc || npm run build || true
echo "[RL.SYS] Sprint 494 Concluída. Gatilho Francesa operacional."
