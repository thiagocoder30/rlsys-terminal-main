...
echo "======================================"
echo " RL.SYS CORE - SPRINT 444"
echo " INSTITUTIONAL STRATEGY ALLOCATION"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/2] Compilando Motor de Alocação e Competição de Estratégias..."
mkdir -p src/domain/decision

cat > src/domain/decision/InstitutionalStrategyAllocationEngine.ts <<'EOF'
/**
...
echo "[2/2] Registrando motor de alocação no controle de versão..."
git add src/domain/decision/InstitutionalStrategyAllocationEngine.ts
git add install/sprints/run-sprint-444-strategy-allocation-engine.sh
git commit -m "feat(decision): implement InstitutionalStrategyAllocationEngine to decouple strategy competition logic from UI layer, formalizing the Phase 3 institutional allocation architecture (Sprint 444)" > /dev/null

echo "======================================"
echo -e "\033[1;32m SPRINT 444 INSTALADA COM SUCESSO \033[0m"
echo " STATUS: MOTOR DE ALOCAÇÃO DE ESTRATÉGIAS ATIVO"
echo "======================================"
