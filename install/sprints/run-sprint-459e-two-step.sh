...
echo "======================================"
echo " RL.SYS CORE - SPRINT 459-E"
echo " TWO-STEP BET VERIFICATION"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/2] Compilando Orquestrador com Confirmação Interativa..."
...
export class LivePaperOrchestrator {
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            // ESTADO DE MÁQUINA: CONFIRMAÇÃO DE APOSTA
            if (this.inputMode === 'CONFIRM_BET') {
                const conf = cmd;
                // Default é 'S' se der apenas Enter
                if (conf === 's' || conf === 'y' || conf === 'sim' || conf === '') {
                    this.processRealSettlement(this.pendingNum!, false); // false = não pulou
                } else {
                    this.processRealSettlement(this.pendingNum!, true);  // true = pulou (n)
                }
            ...
            // PROCESSAMENTO PADRÃO DO NÚMERO
            const numStr = cmd.replace(/[^0-9]/g, ''); // Limpa qualquer letra errada digitada junto
            if (numStr === '') return;
            ...
            if (!isNaN(num) && num >= 0 && num <= 36) {
                ...
                // SE O SISTEMA MANDOU APOSTAR, ELE ENTRA NO MODO DE CONFIRMAÇÃO
                if (this.lastQualification === 'SIM' && this.lastRecommendedStrategy && this.lastRecommendedStake > 0) {
                    ...
                    this.rl.prompt(true);
                    return; // Trava aqui até responder
                } else {
                    // Se o sistema NÃO mandou apostar, ele processa direto e economiza seu tempo
    ...
    private processRealSettlement(num: number, isSkipped: boolean): void {
        ...
        if (this.lastQualification === 'SIM' && this.lastRecommendedStrategy && this.lastRecommendedStake > 0) {
            ...
            if (isSkipped) {
                this.lastActionTakenText = `\x1b[33m[PULOU] Giro ${num} anotado. Aposta ignorada. Banca intacta.\x1b[0m`;
                this.sessionLogs.push(`${timeLog},${num},${this.lastRecommendedStrategy}_SKIPPED,0.00,PULOU,0.00,${this.cooldownGuard.currentBankroll.toFixed(2)},${this.currentVixPercent.toFixed(1)}`);
                return;
            }
            ...
            if (isWin) {
                ...
                this.lastActionTakenText = `\x1b[32m[WIN] Aposta executada no ${num}. Lucro: +R$ ${profitOrLoss.toFixed(2)}\x1b[0m`;
            } else {
            ...
            this.lastActionTakenText = `\x1b[90m[TRACKING] Giro ${num} contabilizado no radar.\x1b[0m`;
    ...
    private renderTerminalHud(): void {
        console.clear();
        console.log('======================================================');
        console.log(' 🛡️  RL.SYS CORE - TACTICAL ORCHESTRATOR [V4.9]');
        console.log('======================================================');
...
echo "[2/2] Compilando motor de interatividade assíncrona..."
npx tsc

echo "======================================"
echo -e "\033[1;32m SPRINT 459-E APLICADA COM SUCESSO \033[0m"
echo " STATUS: TWO-STEP VERIFICATION ATIVADO"
echo "======================================"
