...
echo "======================================"
echo " RL.SYS CORE - SPRINT 466"
echo " HFT POST-MORTEM & FUSION FIX (V5.8)"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    // [NOVO] MÉTRICAS INSTITUCIONAIS HFT
    ...
    // MAPAS FÍSICOS DA ROLETA (INCLUINDO FUSION CORRIGIDA)
    ...
    // [FIX] 19 números: o 23 no centro + 9 vizinhos para cada lado
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (['calibrate', 'clean', 'backtest'].some(c => cmd.startsWith(c))) {
                this.lastActionTakenText = `\x1b[33mComando infraestrutural rodado. Aperte Enter para voltar.\x1b[0m`;
            }
    ...
    private processRealSettlement(num: number, isSkipped: boolean): void {
        ...
        if (this.lastQualification === 'SIM' && this.lastRecommendedStrategy && this.lastRecommendedStake > 0) {
            ...
            // [FIX] FUSION_REDUZIDA AGORA É RACETRACK (19 NÚMEROS)
            else if (id === 'FUSION_REDUZIDA') { 
                isWin = this.SECTOR_FUSION.has(num); profitMultiplier = 0.89; // Lucro proporcional a 19 números
            }
    ...
    // [NOVO] RELATÓRIO INSTITUCIONAL HFT
    ...
    private getPlacementInstruction(strategy: string | null, totalStake: number, hedge: number): string {
        ...
        // [FIX] Instrução correta da FUSION REDUZIDA
    ...
    private renderTerminalHud(): void {
        console.clear();
        console.log('======================================================');
        console.log(' 🛡️  RL.SYS CORE - TACTICAL ORCHESTRATOR [V5.8]');
        console.log('======================================================');
...
echo "[2/2] Compilando motor com Post-Mortem Institucional..."
npx tsc

echo "======================================"
echo -e "\033[1;32m ATUALIZAÇÃO INSTITUCIONAL CONCLUÍDA \033[0m"
echo " STATUS: RELATÓRIO HFT & FUSION (SECTOR 23) ATIVOS"
echo "======================================"
