...
echo "======================================"
echo " RL.SYS CORE - SPRINT 616"
echo " SIGMA ENTERPRISE & MARKOV REACTOR"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR/pwa-terminal"

# 1. CÉREBRO MARKOVIANO (useTacticalEngine.ts)
...
export function useTacticalEngine(initialBankroll = 55.00) {
    ...
    const [actionLogs, setActionLogs] = useState<string[]>(() => loadCache('rl_logs', ['[SISTEMA] Motor Markoviano Online. Fase Burn-in Ativa.']));

    // NOVOS ESTADOS SIGMA ENTERPRISE
    const [burnIn, setBurnIn] = useState<number>(15); // Aquecimento obrigatório (15 giros)
    const [cooldown, setCooldown] = useState<number>(0); // Trava temporária por Drawdown
    
    // MEMÓRIA CONTÍGUA O(1) (Blindagem para o Galaxy A50)
    const markovMatrix = useRef<Float64Array>(new Float64Array(1369)); 
    const pnlHistory = useRef<Record<string, number[]>>({}); // Usado para calcular o Z-Score
    const drawdownTracker = useRef<number[]>([]); // Usado para medir a Velocidade de Drawdown

    // Inicializa as janelas deslizantes
    ...
    const DRAWDOWN_LIMIT = -5.0; // Velocidade letal de perda antes de esfriar a mesa
    ...
    // Atualiza a Cadeia de Markov em O(1)
    ...
    // Função de Previsão do Oráculo baseada em Markov
    ...
    // Calcula o Z-Score (Desvio Padrão) de uma estratégia
    const getZScore = useCallback((strat: string, currentPnL: number) => {
        const history = pnlHistory.current[strat];
        if (history.length < 5) return 0; // Dados insuficientes para Z-Score
    ...
    // ==============================================
    // AVALIAÇÃO DE ESTADO E ORÁCULO MARKOVIANO
    // ==============================================
    useEffect(() => {
        if (timeline.length > 3) {
            ...
            let msg = "";
            if (cooldown > 0) msg = `ALERTA DE DRAWDOWN: Mesa em resfriamento obrigatório. Aguarde ${cooldown} giros.`;
            else if (burnIn > 0) msg = `FASE BURN-IN (AQUECIMENTO): Alimente a máquina com mais ${burnIn} giros de observação.`;
            else if (prediction.predictedNumber !== -1 && prediction.confidence >= 2) {
                msg = `ANÁLISE MARKOV: O giro ${lastNum} possui histórico estatístico forte de atrair o número ${prediction.predictedNumber}.`;
    ...
    // ==============================================
    // REATOR DE ENGAJAMENTO (SIGMA ENTERPRISE)
    // ==============================================
    useEffect(() => {
        ...
        for (const [strat, weight] of Object.entries(shadowWeights)) {
            ...
            if (shadowPnL[strat] < 0) continue; // PnL Gate

            // SIGMA: Avaliação de Confiança Estatística (Z-Score)
            const zScore = getZScore(strat, shadowPnL[strat]);

            // Se o peso atende E o desvio padrão aponta anomalia lucrativa sustentável
    ...
    const handleSpin = useCallback((drawnNumber: number, isFinantial: boolean) => {
        ...
        if (cooldown > 0) {
            setCooldown(c => c - 1);
            if (cooldown === 1) logAction(`[SYS] RESFRIAMENTO CONCLUÍDO. Retomando análise de ataque.`);
        }
        ...
        // Só processa o dano financeiro se foi um "Lançamento Real"
        if (isFinantial && activeStrategy && !isLocked && burnIn === 0 && cooldown === 0) {
            ...
            // Velocidade de Drawdown Tracker
            ...
            if (recentDrawdown < DRAWDOWN_LIMIT) {
                logAction(`[ALERTA] VELOCIDADE DE DRAWDOWN DETECTADA (Perda acumulada rápida). Cooldown ativado.`);
                setCooldown(10); // Trava a máquina por 10 rodadas para esfriar a matriz
                drawdownTracker.current = []; // Reseta o tracker
            }
            ...
            logAction(`[TRACK] Giro ${drawnNumber} mapeado nas Matrizes de Markov.`);
        }

        // Atualização Universal de Pesos e Históricos
        setShadowWeights(prev => {
            ...
            setShadowPnL(prevPnl => {
                ...
                for (const strat of Object.keys(STRATEGY_ZONES)) {
                    ...
                    // Grava o histórico de PnL para o Z-Score (Mantém apenas as últimas 20 amostras para evitar leak)
    ...
    const syncTape = useCallback((nums: number[]) => {
        ...
        logAction(`[SYNC] Cadeia de Markov injetada: ${nums.length} dados lidos.`);
    }, [minChip, logAction]);
    ...
    const setManualBankroll = (val: number) => {
        ...
        setBurnIn(15); setCooldown(0); // Reinicia proteções na troca de banca
        ...
        <div className="flex-none">
            ...
            <form onSubmit={(e) => handleFastInput(e, false)} className="flex gap-2 bg-void p-3 rounded-xl border border-neon/30 shadow-[0_0_15px_rgba(0,255,65,0.05)]">
                ...
                <div className="flex flex-col gap-2 w-24">
                    {/* Botão de Pular funciona SEMPRE */}
                    ...
                    {/* Botão de Lançar travado no Burn-in ou Cooldown */}
...
echo "[RL.SYS] Reator Markoviano Compilado. Arquitetura Sigma Enterprise Acoplada."
echo "======================================"
