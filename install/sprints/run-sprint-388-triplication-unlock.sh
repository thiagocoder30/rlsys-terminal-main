    ...
    private generateNextTrade(): void {
        ...
        if (reversedHistory.length % 3 === 2) {
            ...
            if (inicio !== 0 && confirmacao !== 0) {
                ...
                // ==========================================
                // FIX SPRINT 388: A amostra mínima agora exige 15 trios (45 giros) em vez dos impossíveis 35.
                // ==========================================
            ...
            timeline.forEach(num => { 
                Object.keys(scores).forEach(stratId => { 
                    if (engineStrategies[stratId]) {
                        // Passando o num como lastAnchor fictício para o Cross Grid na simulação (avaliação simplificada no scanner)
