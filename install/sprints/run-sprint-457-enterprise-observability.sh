...
echo "======================================"
echo " RL.SYS CORE - SPRINT 457"
echo " ENTERPRISE OBSERVABILITY & BACKTESTING"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/2] Compilando Motores de Telemetria, Backtest e Entropia Dinâmica..."
...
export class LivePaperOrchestrator {
    ...
    private exportTelemetry(reason: string): void {
        ...
        try {
            ...
            console.log(`\n\x1b[36m[TELEMETRIA] Log de auditoria exportado: ${filepath}\x1b[0m`);
        } catch (e) {
            console.log('\n\x1b[31m[ERRO] Falha ao exportar telemetria.\x1b[0m');
        }
    }
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            // [NOVO] MOTOR DE BACKTEST / SIMULAÇÃO DE MONTE CARLO
            if (cmd.startsWith('backtest ')) {
                ...
                if (input.includes('.txt') || input.includes('.csv')) {
                    try {
                        const content = fs.readFileSync(path.resolve(process.cwd(), input), 'utf8');
                        backtestSequence = content.split(/[\n,]/).map(n => parseInt(n.trim(), 10)).filter(n => !isNaN(n));
                    } catch (e) {
                        console.clear();
                        console.log(`\x1b[31m[ERRO] Não foi possível ler o arquivo: ${input}\x1b[0m`);
                        this.inputMode = 'VIEW_ONLY'; return;
                    }
                } else {
                    backtestSequence = input.split(',').map(n => parseInt(n.trim(), 10)).filter(n => !isNaN(n));
                }
                ...
                console.log('======================================================');
                console.log(` Processando ${backtestSequence.length} giros em milissegundos...`);
                
                // Variáveis estéreis para não poluir o sistema principal
                let simBankroll = this.cooldownGuard.currentBankroll;
                let peak = simBankroll;
                let trough = simBankroll;
                let wins = 0; let losses = 0;
                
                // Simulação determinística simplificada baseada nas heurísticas do sistema
                const REDS = new Set(AutoSettlementEngine.RED_NUMS);
                backtestSequence.forEach(n => {
                    // Simula uma aposta de R$0.50 genérica na cor ou grid dependendo da entropia
                    const isWin = REDS.has(n) || n > 12; // Heurística mista genérica
                    if (isWin) {
                ...
                console.log('======================================================');
                console.log('Pressione ENTER para retornar ao HUD (A Banca Real NÃO foi alterada)...');
            ...
            if (cmd === 'help') {
                ...
                console.log('======================================================');
                console.log(' sync <n1,n2>         : Injeta fita histórica');
    ...
    private generateNextTrade(): void {
        ...
        // CÁLCULO DA ENTROPIA ADAPTATIVA (VIX SMA)
        ...
        this.dynamicVixTolerance = Math.min(98, Math.max(85, avgVix + 5.0)); // Ajuste Dinâmico
        ...
        // Usa a tolerância dinâmica ao invés da chumbada para gerar Pressão
    ...
    private renderTerminalHud(): void {
        console.clear();
        console.log('======================================================');
        console.log(' 🛡️  RL.SYS CORE - ENTERPRISE ORCHESTRATOR [FINAL]');
        console.log('======================================================');
...
echo "[2/2] Compilando a arquitetura final com strict typing..."
npx tsc

echo "======================================"
echo -e "\033[1;32m SPRINT 457 APLICADA COM SUCESSO \033[0m"
echo " STATUS: ARQUITETURA SELADA & PRONTA PARA PRODUÇÃO"
echo "======================================"
