...
echo "======================================"
echo " RL.SYS CORE - SPRINT 464"
echo " DYNAMIC GRID & RISK-ADJUSTED RL"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    // [NOVO] DETECTOR DINÂMICO DE COLUNAS QUENTES
    private getHotColumns(): number[] {
        ...
        return [arr[0].c, arr[1].c]; // Retorna as 2 melhores
    }

    // [NOVO] DETECTOR DINÂMICO DE DÚZIAS QUENTES
    private getHotDozens(): number[] {
        ...
        return [arr[0].d, arr[1].d]; // Retorna as 2 melhores
    }
    ...
    private processRealSettlement(num: number, isSkipped: boolean): void {
        ...
        if (this.lastQualification === 'SIM' && this.lastRecommendedStrategy && this.lastRecommendedStake > 0) {
            ...
            if (num === 0 && id !== 'SECTOR_VOISINS') { isWin = false; } 
                ...
                isWin = hotCols.includes(colNum);
                profitMultiplier = 0.5; // Lucro líquido de cobrir 2 colunas
            }
    ...
    private processShadowTrading(num: number): void {
        ...
        for (const id of Object.keys(this.shadowWeights)) {
            ...
            if (num === 0 && id !== 'SECTOR_VOISINS') isWin = false;
                ...
                const colNum = (num % 3 === 0) ? 3 : (num % 3);
                isWin = hotCols.includes(colNum);
                profitMulti = 0.5; // Ajustado ao risco-retorno real
            }
            else if (id === 'FUSION_REDUZIDA') {
                let dozNum = 0;
                if (num > 0 && num <= 12) dozNum = 1;
            ...
            // [FIX] SHADOW TRADING COM PNL REALISTA MONETÁRIO (RISK-ADJUSTED)
            if (isWin) {
                this.shadowPnL[id] += (1.0 * profitMulti);
                this.shadowWeights[id] = Math.min(2.5, this.shadowWeights[id] * 1.15);
            } else {
                this.shadowPnL[id] -= 1.0; // Perde a unidade inteira de risco
    ...
    private renderTerminalHud(): void {
        console.clear();
        console.log('======================================================');
        console.log(' 🛡️  RL.SYS CORE - TACTICAL ORCHESTRATOR [V5.5]');
        console.log('======================================================');
...
echo "[2/2] Compilando motor Grid Risco-Ajustado..."
npx tsc

echo "======================================"
echo -e "\033[1;32m SPRINT 464 APLICADA COM SUCESSO \033[0m"
echo " STATUS: GRID DINÂMICO E PNL JUSTO ATIVOS"
echo "======================================"
