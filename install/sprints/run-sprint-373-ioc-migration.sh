...
echo "======================================"
echo " RL.SYS CORE - SPRINT 373"
echo " ENTERPRISE ARCHITECTURE & IoC MIGRATION"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/4] Estruturando Contratos de Interface (SOLID: DIP)..."
mkdir -p src/domain/interfaces
cat > src/domain/interfaces/IBankrollRepository.ts <<'EOF'
...
echo "[2/4] Promovendo Rastreador de Histórico para TypeScript com Tipagem Estrita..."
...
echo "[3/4] Construindo Orquestrador Refatorado via Injeção de Dependência (DI)..."
...
echo "[4/4] Estabelecendo o Composition Root (Ponto Único de Entrada DI)..."
mkdir -p src
cat > src/main.ts <<'EOF'
...
echo "Removendo o orquestrador acoplado legado..."
rm -f scripts/live-paper-orchestrator.js

echo "Registrando commit arquitetural..."
git add src/domain/interfaces/ src/domain/analytics/LiveMesaTracker.ts src/presentation/cli/LivePaperOrchestrator.ts src/main.ts
git commit -m "refactor(architecture): implement strict TS typing, Dependency Inversion (DI), and migrate orchestrator to presentation layer (Sprint 373)" > /dev/null

echo "======================================"
echo -e "\033[1;32m SPRINT 373 HOMOLOGADA E INSTALADA \033[0m"
echo " STATUS: ARQUITETURA ENTERPRISE IoC ATIVADA"
echo "======================================"
