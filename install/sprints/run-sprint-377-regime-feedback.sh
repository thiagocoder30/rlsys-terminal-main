...
echo "======================================"
echo " RL.SYS CORE - SPRINT 377"
echo " DYNAMIC STRATEGY EVALUATOR ENGINE"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/3] Construindo o Avaliador de Performance de Regime (Domain Layer)..."
...
echo "[2/3] Executando Testes Unitários de Caixa-Preta (TDD Sandbox)..."
cat > tests/sprint-377-performance.test.ts <<'EOF'
...
# Compila e roda o teste unitário preventivo
npx ts-node tests/sprint-377-performance.test.ts || exit 1

echo "[3/3] Refatorando Orquestrador CLI para acoplar a Malha de Feedback..."
...
echo "[3/3] Commitando novo motor de aprendizado por regime..."
git add src/domain/risk/StrategyPerformanceEvaluator.ts tests/sprint-377-performance.test.ts src/presentation/cli/LivePaperOrchestrator.ts
git commit -m "feat(risk): integrate StrategyPerformanceEvaluator feedback loop to place underperforming signatures into quarantine dynamically (Sprint 377)" > /dev/null

echo "======================================"
echo -e "\033[1;32m SPRINT 377 HOMOLOGADA E INSTALADA \033[0m"
echo " STATUS: CIRCUITO FEEDBACK ADAPTATIVO ATIVO"
echo "======================================"
