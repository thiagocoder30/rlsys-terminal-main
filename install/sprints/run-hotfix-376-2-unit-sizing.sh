...
echo "======================================"
echo " RL.SYS CORE - HOTFIX 376.2"
echo " RISK: STRUCTURAL UNIT SIZING ARCHITECTURE"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/3] Atualizando o Motor de Dimensionamento de Risco (Unit Chip Sizing)..."
cat > src/domain/risk/PositionSizingEngine.ts <<'EOF'
...
echo "[2/3] Atualizando os Testes Unitários de Integração Contínua (TDD)..."
cat > tests/sprint-374-risk.test.ts <<'EOF'
...
# Roda o teste para travar qualquer regressão matemática antes de subir o Orquestrador
npx ts-node tests/sprint-374-risk.test.ts || exit 1

echo "[3/3] Refatorando Orquestrador CLI para acoplar a Nova Escala de Unidades..."
...
echo "[3/3] Registrando e commitando a arquitetura de Unit Chip Sizing..."
git add src/domain/risk/PositionSizingEngine.ts tests/sprint-374-risk.test.ts src/presentation/cli/LivePaperOrchestrator.ts
git commit -m "fix(risk): refactor Fractional Kelly engine to compute dynamic layout stakes as strict multiples of the provider floor chip sizes (Hotfix 376.2)" > /dev/null

echo "======================================"
echo -e "\033[1;32m HOTFIX 376.2 CONFIGURADO E INSTALADO \033[0m"
echo " STATUS: MATRIZ DE FICHAS UNITÁRIAS CORRIGIDA"
echo "======================================"
