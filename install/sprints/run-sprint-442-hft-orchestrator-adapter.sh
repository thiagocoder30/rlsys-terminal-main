...
// FIX SPRINT 442: Importação da Infraestrutura HFT Zero-Allocation
...
export class LivePaperOrchestrator {
    ...
    public async initialize(): Promise<void> {
        ...
        // Inscrição Síncrona O(1) no Event Bus
    ...
    /**
     * Callback Síncrono: Acionado pelo Pipeline HFT sem bloqueio do Event Loop.
     */
    ...
    private generateNextTrade(): void {
        ...
        if (reversedHistory.length % 3 === 2) {
            ...
            if (inicio !== 0 && confirmacao !== 0) {
                ...
                if (colorStats.totalTrios >= 15 && colorStats.dominantRatio >= 0.42) {
                ...
                if (parityStats.totalTrios >= 15 && parityStats.dominantRatio >= 0.42) {
                ...
                if (colorTarget) { prospectiveId = colorTarget === 'A' ? 'TRIPLICACAO_RED' : 'TRIPLICACAO_BLACK'; hftTargetSector = 1; }
                else if (parityTarget) { prospectiveId = parityTarget === 'A' ? 'TRIPLICACAO_EVEN' : 'TRIPLICACAO_ODD'; hftTargetSector = 2; }
        ...
        if (!prospectiveId) {
            ...
            timeline.forEach(num => { 
                Object.keys(scores).forEach(stratId => { 
                    if (engineStrategies[stratId]) {
                        const result = engineStrategies[stratId].evaluate(num, timeline[timeline.indexOf(num)-1] || 0); 
            ...
            if (bestStrat && maxScore >= (timeline.length * 0.40)) { 
                prospectiveId = bestStrat; 
                if (bestStrat === 'CROSS_GRID_HEDGE') {
                    const lastNum = operationalHistory[operationalHistory.length - 1];
                    if (lastNum >= 1 && lastNum <= 12) hftTargetSector = 23;
                    else if (lastNum >= 13 && lastNum <= 24) hftTargetSector = 13;
                    else if (lastNum >= 25 && lastNum <= 36) hftTargetSector = 12;
                }
            }
        }

        // Delegação de Inteligência ao Pipeline HFT (Adapter Link)
        if (prospectiveId && this.performanceEvaluator.isAllowed(prospectiveId) && !this.disabledStrategies.has(prospectiveId)) {
            // Mapeamento dos sinais brutos em convergência simulada da mesa para alimentar o Regime O(1)
            const rawConvergence = 75; // Simulação de proxy estrutural aceitável
            const rawDecision = 70;
            const rawConfidence = 85;
            const rawDecay = -5; // Forçando aprovação condicional baseada na triagem pesada anterior
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (cmd.startsWith('setbankroll ')) {
                ...
                this.saveSystemState(); 
                this.hftPipeline.resetPipeline(); // Zera o EMA de degradação do HFT
                this.evaluateProximityTakeProfit();
                this.generateNextTrade(); 
            ...
            if (cmd === 'undo') {
                ...
                if (history.length > 0) {
                    ...
                    this.saveSystemState(); 
                    if (!this.evaluateProximityTakeProfit()) this.generateNextTrade();
                    console.clear();
                    console.log(`\n \x1b[33m[!] DESFEITO: O número ${removedNum} foi apagado da timeline.\x1b[0m`);
                    if (!this.isDailyHardLocked) setTimeout(() => this.renderTerminalHud(), 1500);
                } else {
                    console.log('\n \x1b[31mA timeline já está vazia.\x1b[0m');
                    if (!this.isDailyHardLocked) setTimeout(() => this.renderTerminalHud(), 1000);
                }
                return;
            }
            ...
            if (cmd === 'reset') {
                ...
                this.saveSystemState(); this.hftPipeline.resetPipeline(); this.generateNextTrade(); this.renderTerminalHud(); return;
            }
            ...
            if (cmd === 'help' || cmd === 'ajuda') {
                ...
                console.log('======================================================');
                console.log(' [ INSERÇÃO E ROTEAMENTO ]');
                console.log(' setbankroll <v>    : Ajusta a banca SEM APAGAR A MESA.');
                console.log(' undo               : Apaga último número digitado.');
                console.log(' disable <id>       : Desliga ESTRATÉGIAS EXATAS.');
                console.log(' enable <id>        : Religa estratégias.');
                console.log(' sync <n,n,...>     : Insere múltiplos números.');
                console.log('\n [ AUDITORIA E ANÁLISE (XAI) ]');
                console.log(' timeline           : Exibe a fita dos últimos 15 giros.');
                console.log(' trios              : Scanner de Padrões (Triplicação).');
                console.log(' stats              : Estatística geral da mesa.');
                console.log(' weights            : Inspeciona os pesos (IDs Exatos).');
                console.log('------------------------------------------------------');
