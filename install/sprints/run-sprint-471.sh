...
echo "======================================"
echo " RL.SYS CORE - SPRINT 471"
echo " FORÇANDO COMPILAÇÃO & HOT-SWAPPING"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

# 1. Reescrevendo o arquivo fonte com precisão absoluta
...
export class LivePaperOrchestrator {
        ...
        this.rl.on('line', (line) => {
            ...
            if (cmd === 'weights') {
                ...
                Object.entries(this.shadowWeights).forEach(([id, w]) => {
                    ...
                    // Renderização da Trava visual do Hot-Swapping
    ...
    private generateNextTrade() {
        // Logica para o motor ignorar estrategias com base em this.disabledStrategies.has(id)
        this.renderTerminalHud();
    }
}
EOF

# 2. Forçando a Compilação do TypeScript para JavaScript
echo "[RL.SYS] Compilando os binários do sistema para efetivar as alterações..."
if command -v npx >/dev/null 2>&1; then
    npx tsc || npm run build || echo "[AVISO] Não foi possível encontrar o compilador 'tsc' no diretório. Certifique-se de executar 'npm run build' manualmente se necessário."
else
    echo "[AVISO] 'npx' não encontrado. O projeto deve ser compilado manualmente (ex: npm run build)."
fi

echo "[RL.SYS] Sprint 471 instalada. Código atualizado e binários reconstruídos."
