...
echo "======================================"
echo " RL.SYS CORE - SPRINT 605"
echo " UX PRECISION & FAST INPUT CONSOLE"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR/pwa-terminal"

# 1. Cérebro atualizado com a Descrição Exata do Engage (activeDesc)
...
export function useTacticalEngine(initialBankroll = 171.00) {
    ...
    useEffect(() => {
        ...
        if (bestStrat) {
            ...
            else if (bestStrat.startsWith('CROSS_')) desc = `Duas Zonas Externas (R$ ${uCost}/cada)`;
...
# 2. Interface Atualizada com Fast Input e Inversão Certa
cat > src/App.tsx <<'EOF'
...
export default function App() {
  ...
  // Timeline render (Invertida: O mais recente fica no índice 0)
  ...
  return (
    <div className="h-[100dvh] w-full bg-obsidian flex flex-col overflow-hidden relative font-mono text-gray-300 select-none">
      ...
      {/* ZONA B: Radar Térmico Invertido */}
          ...
          {displayTimeline.length === 0 ? (
            ...
            displayTimeline.map((num, idx) => (
              <div key={idx} className={`shrink-0 w-10 h-10 flex items-center justify-center rounded border bg-obsidian text-base font-bold relative ${getNumberColor(num)}`}>
                {/* Radar piscando no último número inserido */}
                {idx === 0 && (
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 pb-28 z-0">
        {engine.isLocked ? (
           <div className="bg-blood/10 border border-blood rounded-lg p-4 mb-4 flex flex-col items-center justify-center text-center">
            ...
            <div className="bg-steel/10 border border-neon/50 rounded-lg p-3 mb-4 shadow-[0_0_15px_rgba(0,255,65,0.1)] transition-all">
                ...
                {/* Descrição Corrigida: Exibe exatamente onde e quanto colocar */}
                <div className="text-[10px] text-neon font-bold mt-2 font-sans bg-obsidian p-2 rounded">{engine.activeDesc}</div>
            </div>
        ) : (
        ...
        {/* FAST INPUT BAR (Substitui o toggle de pular) */}
...
echo "[RL.SYS] Precisão de UX aplicada. HMR em andamento..."
echo "======================================"
