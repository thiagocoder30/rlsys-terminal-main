...
echo "======================================"
echo " RL.SYS CORE - SPRINT 484"
echo " KELLY DISCRETO & POSITION SIZING"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    private evaluateShadowTrading(drawnNumber: number) {
        ...
        for (const strat of Object.keys(this.STRATEGY_ZONES)) {
            ...
            // Avaliação sempre em base unitária (Multiplier = 1) para não distorcer os pesos
            if (strat.startsWith('CROSS_GRID')) {
    ...
    private calculateSizing(strategyId: string, minChip: number, weight: number): { total: number, desc: string, multiplier: number } {
        // Pseudo-Kelly Fraction: Transforma o peso RL (ex: 2.5) em porcentagem de banca alvo (2.5%)
        ...
        // Multiplicador Discreto de Kelly (garante precisão na corretora)
        let multiplier = Math.floor(targetStake / baseCost);
        if (multiplier < 1) multiplier = 1; // Trava mínima de segurança
        ...
        this.rl.on('line', (line) => {
            ...
            if (cmd.startsWith('setmacro ')) {
                ...
                if (!isNaN(newVal) && newVal > 0) {
                    ...
                    this.lastActionTakenText = `\x1b[32m[SISTEMA] Macro recalibrado para R$ ${newVal.toFixed(2)}.\x1b[0m`;
                    this.generateNextTrade();
                } else {
                    this.lastActionTakenText = "\x1b[31m[ERRO] Valor inválido.\x1b[0m";
                    this.renderTerminalHud();
...
echo "[RL.SYS] Compilando Dimensionamento Discreto de Kelly..."
npx tsc || npm run build || true
echo "[RL.SYS] Sprint 484 Concluída. Gestão de Banca agressiva online."
