...
echo "======================================"
echo " RL.SYS CORE - SPRINT 606"
echo " PERSISTENT MEMORY & EXACT CALLOUTS"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR/pwa-terminal"

# 1. Cérebro atualizado com LocalStorage e Descrições Explícitas
...
export function useTacticalEngine(initialBankroll = 171.00) {
    // Estados inicializando do LocalStorage (Memória Persistente)
    ...
    // Estados voláteis (não precisam ir para o cache, são calculados na hora)
    ...
    // Efeito GATILHO: Salva tudo no disco do celular a cada batida de pulso (mudança de estado)
    useEffect(() => {
    ...
    }, [bankroll, peakBankroll, timeline, shadowWeights, shadowPnL, sessionWins, sessionLosses]);
    ...
    useEffect(() => {
        ...
        if (bestStrat) {
            ...
            const uCost = (MIN_CHIP * multiplier).toFixed(2);

            // Mapeamento Explícito de Nomenclatura Tática
            let desc = '';
    ...
    return {
        ...
        shadowWeights, shadowPnL, sessionWins, sessionLosses, processSpin, skipSpin, undoSpin, syncTape, setManualBankroll
    };
}
EOF

# 2. Interface Atualizada com o Comando "reset"
cat > src/App.tsx <<'EOF'
    ...
    <div className="h-[100dvh] w-full bg-obsidian flex flex-col overflow-hidden relative font-mono text-gray-300 select-none">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 pb-28 z-0">
        {engine.isLocked ? (
           <div className="bg-blood/10 border border-blood rounded-lg p-4 mb-4 flex flex-col items-center justify-center text-center">
            ...
            <div className="bg-steel/10 border border-neon/50 rounded-lg p-3 mb-4 shadow-[0_0_15px_rgba(0,255,65,0.1)] transition-all">
                ...
                <div className="text-[10px] text-neon font-bold mt-2 font-sans bg-obsidian border border-neon/30 p-2 rounded tracking-widest">{engine.activeDesc}</div>
            </div>
        ) : (
        ...
        <div className="flex justify-between items-center mb-3">
          <span className="text-[10px] text-gray-500 font-sans uppercase">Ação Manual (Aposta Direta)</span>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {activeModal === 'CONFIG' && (
              <div className="text-sm text-gray-400 space-y-4">
                <p>Comandos Administrativos via Terminal:</p>
                <div className="bg-steel/10 p-3 rounded font-mono text-xs text-white">setbankroll [valor]</div>
                <div className="bg-steel/10 p-3 rounded font-mono text-xs text-white">clear (limpa log visual)</div>
                <div className="bg-blood/20 border border-blood p-3 rounded font-mono text-xs text-blood mt-4">reset (Apaga cache e zera a sessão)</div>
              </div>
            )}
          </div>
        </div>
...
echo "[RL.SYS] Persistência Ativada e Descrições Explícitas Aplicadas."
echo "======================================"
