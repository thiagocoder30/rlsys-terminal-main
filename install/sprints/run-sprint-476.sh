...
echo "======================================"
echo " RL.SYS CORE - SPRINT 476"
echo " LIQUIDAÇÃO FINANCEIRA & PAYOUTS ESTREITOS"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    // Tracking financeiro de mercado aberto
    ...
    private readonly STRATEGY_ZONES: Record<string, number[]> = {
        ...
        'CROSS_GRID_HEDGE': [2,5,8,11,14,17,20,23,26,29,32,35, 3,6,9,12,15,18,21,24,27,30,33,36, 0] // Col 2, Col 3 + Zero
    };
    ...
    private resolveFinancials(drawnNumber: number) {
        ...
        if (strat === 'CROSS_GRID_HEDGE') {
            // Estrutura: 10 unidades ColA, 10 unidades ColB, 1 unidade Zero = 21 unidades custo
            const cost = 21 * chip;
            if (!isWin) {
                pnl = -cost;
            } else if (drawnNumber === 0) {
                pnl = (36 * chip) - cost; // Ganho no Zero
            } else {
                pnl = (30 * chip) - cost; // Ganho na Coluna (10 fichas pagam 30)
            }
        } else if (strat.startsWith('SECTOR_') || strat === 'FUSION_REDUZIDA') {
            // Número exato de fichas puras
            ...
            // Aposta externa (18 números paga 1:1, ou seja aposta 18 fichas ganha 36)
            const cost = 18 * chip;
            if (isWin) {
                pnl = (36 * chip) - cost;
            } else {
        ...
        this.activeBet = null; // Zera a aposta
    }
        ...
        this.rl.on('line', (line) => {
            ...
            if (cmd === 'undo') {
                ...
                if (internalHistory && internalHistory.length > 0) {
                    internalHistory.pop();
                    this.activeBet = null; // Cancela apostas pendentes no undo
            ...
            if (cmd === 'help' || cmd === 'weights' || cmd === 'journey' || cmd === 'stats') {
                this.lastActionTakenText = `\x1b[33m[SISTEMA] Telas auxiliares suspensas em modo Live para foco em PnL.\x1b[0m`;
                this.renderTerminalHud(); return;
            }
            ...
            if (!isNaN(num) && num >= 0 && num <= 36) {
                if (!isSkipped) {
                    this.resolveFinancials(num); // Processa ganhos/perdas antes de analisar a próxima jogada
                } else {
                    this.activeBet = null; // Se pular a rodada, anula apostas correntes do simulador
    ...
    private generateNextTrade(isSkipped: boolean = false) {
        ...
        if (isSkipped) {
            ...
            if (this.activeStrategyId !== 'Nenhuma') {
                ...
                // Armazena a aposta pendente para o PRÓXIMO giro abater na banca
...
echo "[RL.SYS] Compilando e conectando motor financeiro..."
npx tsc || npm run build || true
echo "[RL.SYS] Sprint 476 finalizada."
