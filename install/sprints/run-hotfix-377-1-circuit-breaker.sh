...
echo "======================================"
echo " RL.SYS CORE - HOTFIX 377.1"
echo " RISK: INSTANT SIGNAL CIRCUIT BREAKER"
echo "======================================"

ROOT_DIR=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
cd "$ROOT_DIR"

echo "[1/3] Elevando a penalidade do Avaliador de Performance (Silenciamento no 1º Loss)..."
...
echo "[2/3] Integrando Quarentena Persistente no Core do Orquestrador..."
...
echo "[3/3] Registrando o Circuit Breaker agressivo na árvore Git..."
git add src/domain/risk/StrategyPerformanceEvaluator.ts src/presentation/cli/LivePaperOrchestrator.ts
git commit -m "fix(risk): implement 1-loss instant circuit breaker calibration and secure cross-session weight state serialization (Hotfix 377.1)" > /dev/null

echo "======================================"
echo -e "\033[1;32m HOTFIX 377.1 HOMOLOGADO E OPERACIONAL \033[0m"
echo " STATUS: CIRCUITO PROTETOR DE SINAL ATIVADO"
echo "======================================"
