...
export class LivePaperOrchestrator {
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (cmd.startsWith('enable ')) {
                const targetId = line.replace(/enable\s+/i, '').trim().toUpperCase();
                this.disabledStrategies.delete(targetId);
                this.generateNextTrade();
                return;
            }
            ...
            if (cmd.startsWith('disable ')) {
                const targetId = line.replace(/disable\s+/i, '').trim().toUpperCase();
                this.disabledStrategies.add(targetId);
                this.generateNextTrade();
                return;
            }
            ...
            if (cmd === 'weights') {
                console.clear();
                console.log('======================================================');
                console.log(' ⚖️  RL.SYS CORE - STRATEGY PERFORMANCE WEIGHTS');
                console.log('======================================================');
                const weights = this.performanceEvaluator.getAllWeights() as Record<string, number>;
                Object.entries(weights).forEach(([id, w]) => {
                    const isStratDisabled = this.disabledStrategies.has(id);
                    const statusLabel = isStratDisabled ? '\x1b[31m[DESABILITADA]\x1b[0m' : '\x1b[32m[ATIVA]\x1b[0m';
                    console.log(`  Estratégia: ${id.padEnd(22)} | Peso: ${Number(w).toFixed(2)} | Status: ${statusLabel}`);
                });
                console.log('======================================================');
            ...
            if (cmd === 'help') {
                console.clear();
                console.log('======================================================');
                console.log(' 🧠 RL.SYS CORE - COMANDOS DE GOVERNANÇA TÁTICA');
                console.log('======================================================');
                console.log(' sync <n1,n2...>   : Injeta fita de dados histórica (Warmup)');
                console.log(' undo              : Remove o último número inserido da janela');
                console.log(' stats             : Exibe distribuição de frequências topográficas');
                console.log(' trios             : Exibe análise de entropia por blocos curtos');
                console.log(' weights           : Exibe pesos de performance e status atual');
                console.log(' enable <ID>       : Reabilita uma estratégia na competição');
                console.log(' disable <ID>      : Bloqueia/Desabilita uma estratégia por ID');
                console.log(' exit / quit       : Encerra a sessão com salvamento estrito');
                console.log('======================================================');
                console.log('Pressione ENTER para retornar ao HUD...');
                this.inputMode = 'VIEW_ONLY';
                return;
    ...
    private updateXaiTranslation(prospectiveId: string | null): void {
        ...
        if (this.liveExecutionPressure === 'AGGRESSIVE_ENTRY') {
            ...
            this.xaiReason = 'Regime confirmed + janela aberta + baixa degradação temporal.';
