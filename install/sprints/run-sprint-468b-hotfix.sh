...
echo "======================================"
echo " RL.SYS CORE - HOTFIX 468-B"
echo " RESTAURAÇÃO DO MOTOR DE BACKTEST (V5.10b)"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    public async initialize(): Promise<void> {
        ...
        baseStrats.forEach(id => { this.shadowWeights[id] = 1.0; this.shadowPnL[id] = 0.0; });
    ...
    private broadcastLiveTelemetry(): void {
        ...
        const payload = {
            timestamp: Date.now(), bankroll: this.cooldownGuard.currentBankroll,
            takeProfit: this.takeProfitM3, stopLoss: this.hardStopLoss,
            vix: this.currentVixPercent, strategy: this.activeStrategyId || 'Aguardando',
            stake: this.dynamicStakeCalculated, qualification: this.xaiQualification,
            pnlCurve: this.pnlCurve, recentSpins: this.mesaTracker.getHistory().slice(-30),
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (this.inputMode === 'CONFIRM_BET') {
                ...
                } else { this.processRealSettlement(this.pendingNum!, true); }
                this.processShadowTrading(this.pendingNum!);
                this.generateNextTrade(); this.inputMode = 'NUMBER'; this.pendingNum = null; return;
            }
            ...
            if (cmd === 'journey') {
                ...
                const m1 = this.macroBaseline * 2; const m2 = this.macroBaseline * 5; const m3 = this.macroBaseline * 10;
            ...
            if (cmd.startsWith('setmacro ')) {
                const valStr = cmd.replace('setmacro ', '').trim(); const newVal = parseFloat(valStr);
            ...
            if (cmd.startsWith('setbankroll ')) {
                const valStr = cmd.replace('setbankroll ', '').trim(); const newVal = parseFloat(valStr);
                if (!isNaN(newVal) && newVal >= 0) {
                    this.initialBankroll = newVal; this.sessionPeak = newVal; this.sessionTrough = newVal;
                    this.cooldownGuard = new DynamicEmotionalCooldownGuard(this.initialBankroll, this.savedState);
                    this.cooldownGuard.currentBankroll = newVal; this.pnlCurve = [newVal]; 
                    const totalTargetGain = this.cooldownGuard.nextMilestone - this.initialBankroll;
                    this.takeProfitM3 = this.initialBankroll + (totalTargetGain * 0.75);
                    this.hardStopLoss = this.initialBankroll * 0.85; 
            ...
            if (cmd.startsWith('sync ')) {
                ...
                nums.forEach(n => {
                    if (!isNaN(n) && n >= 0 && n <= 36) {
                        this.mesaTracker.addNumber(n); this.localHistoryCache.push(n);
                        this.processShadowTrading(n); this.pnlCurve.push(this.cooldownGuard.currentBankroll);
                    }
                });
                this.lastActionTakenText = '\x1b[36mSincronização concluída.\x1b[0m'; this.generateNextTrade(); return;
            }

            // O RETORNO DA INFRAESTRUTURA DE AUDITORIA
            if (cmd === 'stats') {
                console.clear(); const history = this.mesaTracker.getHistory(); const total = history.length;
                console.log('======================================================');
                console.log(' 📊 ESTATÍSTICAS DA MESA & HEATMAP');
                console.log('======================================================');
                if (total === 0) { console.log(' \x1b[31mNenhum histórico registrado ainda.\x1b[0m'); } 
                else {
                    const REDS = new Set(AutoSettlementEngine.RED_NUMS);
                    let redC = 0, blackC = 0, zeroC = 0, evenC = 0, oddC = 0; const freqs = new Array(37).fill(0);
                    history.forEach(n => { freqs[n]++; if (n === 0) zeroC++; else { if (REDS.has(n)) redC++; else blackC++; if (n % 2 === 0) evenC++; else oddC++; } });
                    ...
                    const formatCell = (n: number) => {
                        const f = freqs[n]; const str = n.toString().padStart(2, ' ');
                ...
                console.log('======================================================');
                console.log('Pressione ENTER para retornar ao HUD...'); this.inputMode = 'VIEW_ONLY'; return;
            }
            ...
            if (cmd.startsWith('calibrate ')) {
                ...
                try {
                    ...
                    if (redMatch && blackMatch) {
                        const redPct = parseFloat(redMatch[1]); const blackPct = parseFloat(blackMatch[1]);
                        console.log(` Leitura Macro: VERMELHO ${redPct.toFixed(1)}% | PRETO ${blackPct.toFixed(1)}%`);
                        if (redPct > 49.5) this.shadowWeights['TRIPLICACAO_RED'] = 1.30; else if (redPct < 47.5) this.shadowWeights['TRIPLICACAO_RED'] = 0.70;
                        if (blackPct > 49.5) this.shadowWeights['TRIPLICACAO_BLACK'] = 1.30; else if (blackPct < 47.5) this.shadowWeights['TRIPLICACAO_BLACK'] = 0.70;
                        console.log(`\x1b[32m [!] Pesos calibrados com sucesso.\x1b[0m`);
                    } else { console.log('\x1b[31m[ERRO] Estrutura incorreta.\x1b[0m'); }
                } catch (e) { console.log(`\x1b[31m[ERRO] Falha ao ler.\x1b[0m`); }
                console.log('======================================================');
            ...
            if (cmd.startsWith('clean ')) {
                ...
                try {
                    const rawContent = fs.readFileSync(path.resolve(process.cwd(), targetFile), 'utf8');
                    let validSpins: number[] = []; const jsonRegex = /"(?:result|number|value|spin)"\s*:\s*(\d+)/gi;
                    let match; let foundJson = false;
                    while ((match = jsonRegex.exec(rawContent)) !== null) { foundJson = true; const num = parseInt(match[1], 10); if (num >= 0 && num <= 36) validSpins.push(num); }
                    if (!foundJson) {
                        const lines = rawContent.split(/\r?\n/);
                        lines.forEach(line => { const cleanLine = line.trim(); if (/^\d{1,2}$/.test(cleanLine)) { const num = parseInt(cleanLine, 10); if (num >= 0 && num <= 36) validSpins.push(num); }});
                    }
                    if (validSpins.length > 0) {
                        validSpins.reverse(); const outPath = path.join(process.cwd(), 'data', 'fita_limpa.txt');
                        fs.writeFileSync(outPath, validSpins.join(',')); console.log(`\x1b[32m[SUCESSO] ${validSpins.length} giros extraídos!\x1b[0m`);
                    }
            ...
            if (cmd.startsWith('backtest ')) {
                const input = line.replace(/backtest\s+/i, '').trim(); let backtestSequence: number[] = [];
                try {
                    let content = '';
                ...
                } catch (e) { console.clear(); console.log(`\x1b[31m[ERRO] Falha no arquivo de backtest.\x1b[0m`); this.inputMode = 'VIEW_ONLY'; return; }
                ...
                let peak = simBankroll; let trough = simBankroll; let wins = 0; let losses = 0;
                
                const simShadow: Record<string, number> = {};
                const baseStrats = Object.keys(AutoSettlementEngine.getStrategies());
                baseStrats.push('SECTOR_VOISINS', 'SECTOR_TIERS', 'SECTOR_ORPHELINS', 'FUSION_REDUZIDA');
                baseStrats.forEach(s => simShadow[s] = 1.0);
                
                const REDS = new Set(AutoSettlementEngine.RED_NUMS); const simHistory: number[] = [];
                
                backtestSequence.forEach(n => {
                    let bStrat: string | null = null; let bW = -1;
                    Object.keys(simShadow).forEach(s => { if(simShadow[s]>bW){bW=simShadow[s]; bStrat=s;} });
                    let hc1=0,hc2=0,hc3=0;
                    simHistory.slice(-20).forEach(hn => { if(hn!==0){ if(hn%3===1)hc1++; else if(hn%3===2)hc2++; else hc3++; }});
                    const hCols = [{c:1,v:hc1},{c:2,v:hc2},{c:3,v:hc3}].sort((a,b)=>b.v-a.v).map(o=>o.c).slice(0,2);
                    
                    let isWin = false;
                    if(bStrat) {
                        if (n === 0 && bStrat !== 'SECTOR_VOISINS') isWin = false;
                        else if (bStrat === 'TRIPLICACAO_RED') isWin = REDS.has(n);
                        else if (bStrat === 'TRIPLICACAO_BLACK') isWin = (!REDS.has(n) && n !== 0);
                        else if (bStrat === 'TRIPLICACAO_EVEN') isWin = (n % 2 === 0 && n !== 0);
                        else if (bStrat === 'TRIPLICACAO_ODD') isWin = (n % 2 !== 0);
                        else if (bStrat === 'CROSS_GRID_HEDGE') { const cNum = (n % 3 === 0) ? 3 : (n % 3); isWin = hCols.includes(cNum); }
                        else if (bStrat === 'FUSION_REDUZIDA') isWin = this.SECTOR_FUSION.has(n);
                        else if (bStrat === 'SECTOR_VOISINS') isWin = this.SECTOR_VOISINS.has(n);
                        else if (bStrat === 'SECTOR_TIERS') isWin = this.SECTOR_TIERS.has(n);
                        else if (bStrat === 'SECTOR_ORPHELINS') isWin = this.SECTOR_ORPHELINS.has(n);
                    }
                    ...
                    simHistory.push(n);
                    Object.keys(simShadow).forEach(id => {
                        let w = false; let pM=1.0;
                        if (n===0 && id!=='SECTOR_VOISINS') w=false;
                        else if (id==='TRIPLICACAO_RED') {w=REDS.has(n); pM=1.0;}
                        else if (id==='TRIPLICACAO_BLACK') {w=(!REDS.has(n)&&n!==0); pM=1.0;}
                        else if (id==='TRIPLICACAO_EVEN') {w=(n%2===0&&n!==0); pM=1.0;}
                        else if (id==='TRIPLICACAO_ODD') {w=(n%2 !== 0); pM=1.0;}
                        else if (id==='CROSS_GRID_HEDGE') { const c=(n%3===0)?3:(n%3); w=hCols.includes(c); pM=0.5; }
                        else if (id==='FUSION_REDUZIDA') { w=this.SECTOR_FUSION.has(n); pM=0.89; }
                        else if (id==='SECTOR_VOISINS') {w=this.SECTOR_VOISINS.has(n); pM=0.8;}
                        else if (id==='SECTOR_TIERS') {w=this.SECTOR_TIERS.has(n); pM=2.0;}
                        else if (id==='SECTOR_ORPHELINS') {w=this.SECTOR_ORPHELINS.has(n); pM=3.5;}
                        
                        if(w) simShadow[id] = Math.min(2.5, simShadow[id]*1.15);
                        else simShadow[id] = Math.max(0.1, simShadow[id]*0.85);
                    });
                });
                
                const winRate = wins + losses > 0 ? (wins / (wins + losses)) * 100 : 0;
                const maxDrawdown = peak - trough; const pnl = simBankroll - this.cooldownGuard.currentBankroll;
                console.log(` \n --- RELATÓRIO DE SIMULAÇÃO ---`);
                console.log(` Win Rate Bruto  : ${winRate.toFixed(1)}% (${wins}W / ${losses}L)`);
                console.log(` Max Drawdown    : -R$ ${maxDrawdown.toFixed(2)}`);
                console.log(` PnL Projetado   : R$ ${pnl.toFixed(2)}`);
            ...
            if (cmd === 'undo') {
                if (this.localHistoryCache.length > 0) {
                    this.localHistoryCache.pop(); const internalHistory = (this.mesaTracker as any).history;
                    if (internalHistory && Array.isArray(internalHistory)) internalHistory.pop();
                    this.lastActionTakenText = '\x1b[33mÚltimo número removido da fita.\x1b[0m'; this.generateNextTrade();
                }
                return;
            }
    ...
    private processRealSettlement(num: number, isSkipped: boolean): void {
        ...
        if (this.lastQualification === 'SIM' && this.lastRecommendedStrategy && this.lastRecommendedStake > 0) {
            if (isSkipped) {
                ...
                this.pnlCurve.push(this.cooldownGuard.currentBankroll); return;
            }

            const REDS = new Set(AutoSettlementEngine.RED_NUMS);
            let isWin = false; let profitMultiplier = 1.0;
            const id = this.lastRecommendedStrategy; const hotCols = this.getHotColumns();
    ...
    private renderTerminalHud(): void {
        ...
        let nextTarget = mstones[mstones.length - 1]; let prevTarget = this.macroBaseline;
        for (let i = 0; i < mstones.length; i++) {
            if (current < mstones[i]) { nextTarget = mstones[i]; if (i > 0) prevTarget = mstones[i-1]; break; }
        }
        let progressPct = 0;
        ...
        console.clear();
        console.log('======================================================');
        console.log(' 🛡️  RL.SYS CORE - TACTICAL ORCHESTRATOR [V5.10b]');
        console.log('======================================================');
...
echo "[2/2] Compilando Orquestrador com Infraestrutura Total..."
npx tsc

echo "======================================"
echo -e "\033[1;32m COMPILAÇÃO V5.10b CONCLUÍDA \033[0m"
echo " STATUS: BACKTEST & STATS RESTAURADOS"
echo "======================================"
