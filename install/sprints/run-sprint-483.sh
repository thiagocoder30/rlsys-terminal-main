...
echo "======================================"
echo " RL.SYS CORE - SPRINT 483"
echo " RESTAURAÇÃO DA MALHA CROSS GRID DINÂMICA"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    private calculateSizing(strategyId: string, minChip: number): { total: number, desc: string } {
        if (strategyId.startsWith('CROSS_GRID')) {
            ...
            return { total, desc: `\x1b[32m${colsDesc} (R$ ${col.toFixed(2)} cada), Zero (R$ ${zero.toFixed(2)})\x1b[0m` };
        } 
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (cmd.startsWith('sync ')) {
                ...
                this.lastActionTakenText = '\x1b[36m[SISTEMA] Fita histórica injetada. Pesos limpos e recalibrados.\x1b[0m';
                this.generateNextTrade(); 
                return;
            }
            ...
            if (cmd.startsWith('setbankroll ')) {
                ...
                if (!isNaN(newVal) && newVal >= 0) {
                    ...
                    this.lastActionTakenText = "\x1b[31m[ERRO] Valor inválido para setbankroll.\x1b[0m";
                    this.renderTerminalHud();
                }
                return;
            }
            if (cmd.startsWith('setmacro ')) {
                ...
                if (!isNaN(newVal) && newVal > 0) {
                    ...
                    this.lastActionTakenText = "\x1b[31m[ERRO] Valor inválido para setmacro.\x1b[0m";
                    this.renderTerminalHud();
                }
                return;
            }
            ...
            if (cmd === 'stats' || cmd.startsWith('backtest')) {
                this.lastActionTakenText = `\x1b[33m[SISTEMA] Módulo em manutenção. Foco em Live PnL.\x1b[0m`;
                this.renderTerminalHud(); return;
            }
            if (cmd === 'undo') {
    ...
    private generateNextTrade() {
        ...
        if (this.currentVixPercent > 70) {
            ...
            if (this.activeStrategyId !== 'Nenhuma') {
                ...
                this.xaiReason = `Estratégia blindada pelo Shadow Trading (Peso: ${this.shadowWeights[this.activeStrategyId].toFixed(2)}).`;
...
echo "[RL.SYS] Compilando a Fragmentação da Malha Dinâmica..."
npx tsc || npm run build || true
echo "[RL.SYS] Sprint 483 Concluída."
