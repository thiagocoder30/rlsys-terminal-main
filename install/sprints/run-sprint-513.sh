#!/bin/bash
# RL.SYS Core V5.10b - Sprint Installer (Sprint 513)
# Modulo: Parsing Universal de Comandos & Isolamento Absoluto de Motor

echo "[RL.SYS] Iniciando integracao da Sprint 513..."
sleep 1
...
class RLSysCore:
    ...
    def run_backtest(self, file_path):
        ...
        # Prova visual de que o motor cortou a estrategia desabilitada
        for s in active_strats:
            print(f"  -> {s} [ONLINE]")
            
        time.sleep(1)
        print("\n[SISTEMA] Simulacao de Backtest concluida.")
EOF
...
class SystemHUD:
    ...
    def cmd_weights(self):
        ...
        for strat, data in self.core.strategy_manager.strategies.items():
            ...
            pnl_str = f"+{data['pnl']:.2f}" if data['pnl'] > 0 else f"{data['pnl']:.2f}"
    ...
    def parse_strategy_name(self, args):
        # Formatador Absoluto: Une espacos, substitui hifens e forca caixa alta.
        raw = "_".join(args).upper()
    ...
    def execute_command(self, command_line):
        ...
        if cmd == "setbankroll" and args: self.core.bankroll = float(args[0])
            ...
            if self.core.strategy_manager.disable_strategy(strat):
                ...
                print(f"\n[ERRO] Estratégia '{strat}' nao encontrada na ID Table.")
            time.sleep(1.5)
...
echo "[RL.SYS] Sprint 513 aplicada com sucesso. O Motor V5.10b esta 100% selado."
