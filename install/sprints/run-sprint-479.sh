...
echo "======================================"
echo " RL.SYS CORE - SPRINT 479"
echo " RESTAURAÇÃO DO SHADOW TRADING E PESOS"
echo "======================================"
...
export class LivePaperOrchestrator {
    ...
    private resolveFinancials(drawnNumber: number) {
        ...
        if (strat === 'CROSS_GRID_HEDGE') {
            const cost = 21 * chip;
            if (!isWin) pnl = -cost;
            else if (drawnNumber === 0) pnl = (36 * chip) - cost; 
            else pnl = (30 * chip) - cost; 
        } else if (strat.startsWith('SECTOR_') || strat === 'FUSION_REDUZIDA') {
            const cost = zone.size * chip;
            pnl = isWin ? (36 * chip) - cost : -cost;
    ...
    private evaluateShadowTrading(drawnNumber: number) {
        ...
        for (const strat of Object.keys(this.STRATEGY_ZONES)) {
            ...
            if (pnl > 0) {
                this.shadowWeights[strat] = Math.min(3.0, (this.shadowWeights[strat] || 1.0) + 0.15);
            } else {
                this.shadowWeights[strat] = Math.max(0.1, (this.shadowWeights[strat] || 1.0) - 0.05);
            }
        }
    }
    ...
    private attachEventListeners(): void {
        this.rl.on('line', (line) => {
            ...
            if (cmd.startsWith('sync ')) {
                ...
                this.lastActionTakenText = '\x1b[36m[SISTEMA] Fita histórica injetada. Pesos RL recalibrados.\x1b[0m';
                this.generateNextTrade(); 
                return;
            }
            ...
            if (cmd === 'weights') {
                ...
                console.log('======================================================');
                const sorted = Object.entries(this.shadowWeights).sort((a, b) => b[1] - a[1]);
                sorted.forEach(([id, w]) => {
                    ...
                    const sign = pnl > 0 ? '+' : '';
                    console.log(` ${status} Estratégia: ${id.padEnd(20)} | PnL Monetário: ${pnlColor}${sign}${pnl.toFixed(2)}\x1b[0m | Peso RL: ${Number(w).toFixed(2)}`);
                });
                console.log('======================================================');
                console.log('Pressione ENTER para retornar ao HUD...');
                this.inputMode = 'VIEW_ONLY'; return;
            }
            if (cmd === 'help' || cmd === 'journey' || cmd === 'stats' || cmd.startsWith('set')) {
                this.lastActionTakenText = `\x1b[33m[SISTEMA] Comando '${cmd.split(' ')[0]}' processado. Tela em foco Live.\x1b[0m`;
                this.renderTerminalHud(); return;
            }
    ...
    private generateNextTrade() {
        ...
        if (this.currentVixPercent > 70) {
            ...
            if (this.activeStrategyId !== 'Nenhuma') {
                ...
                this.xaiReason = `Estratégia promovida pelo Shadow Trading (Peso: ${this.shadowWeights[this.activeStrategyId].toFixed(2)}).`;
                ...
                this.xaiReason = 'VIX Alto, mas não há reatores (Estratégias) ligados.';
...
echo "[RL.SYS] Compilando IA de Shadow Trading..."
npx tsc || npm run build || true
echo "[RL.SYS] Sprint 479 Concluída. Predador Alfa online."
