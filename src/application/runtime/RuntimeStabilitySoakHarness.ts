export type RuntimeSoakPressureLevel =
  | "LOW"
  | "ELEVATED"
  | "HIGH"
  | "CRITICAL";
...
export interface RuntimeSoakWorkloadPort {
  execute(
    iteration: number,
  ): Promise<RuntimeSoakIterationSample>;
}
...
export interface RuntimeSoakConfiguration {
  ...
  readonly forbiddenPressure:
    RuntimeSoakPressureLevel;
}
 ...
 *
 * Heap drift represents positive memory growth above the initial baseline.
 * If the final heap is below the baseline, drift is zero rather than negative.
 *
 * This matches the memory-pressure classifier semantics and the persisted
 * endurance-report contract, where heap drift is finite and non-negative.
 *
 * Complexity:
 * - O(n), where n is configured iteration count.
 * - Memory O(1), stores only aggregate metrics.
 */
export class RuntimeStabilitySoakHarness {
  public constructor(
    private readonly workload:
      RuntimeSoakWorkloadPort,
  ) {}
  ...
  public async run(
    ...
    for (
      ...
      if (
        this.isPressureViolation(
          sample.pressure,
          configuration.forbiddenPressure,
        )
      ) {
    ...
    /*
     * Drift means positive growth above baseline.
     *
     * A lower final heap after GC represents no positive drift.
     */
    ...
    if (
      heapDriftBytes >
      configuration.maxHeapDriftBytes
    ) {
    ...
    if (
      peakEventLoopLagMs >
      configuration.maxPeakEventLoopLagMs
    ) {
      violationMessages.push(
  ...
  private validateConfiguration(
    ...
    if (
      ...
      throw new Error(
        "Invalid soak configuration: iterations must be a positive integer.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Invalid soak configuration: maxHeapDriftBytes must be non-negative.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Invalid soak configuration: maxPeakEventLoopLagMs must be non-negative.",
      );
    }
  }
  ...
  private validateSample(
    ...
    if (
      ...
      throw new Error(
        "Invalid soak sample: iteration sequence mismatch.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Invalid soak sample: heapUsedBytes must be non-negative.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Invalid soak sample: eventLoopLagMs must be non-negative.",
      );
    }
  }
