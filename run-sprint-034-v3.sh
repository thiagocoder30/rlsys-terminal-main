...
PATCH="rlsys-terminal-sprint-034-runtime-enforcement-orchestrator-v3.diff"
BRANCH="sprint-034-runtime-enforcement-orchestrator"
LOG_DIR="$PROJECT_DIR/logs"
RUN_LOG="$LOG_DIR/sprint-034-v3-install-$(date +%Y%m%d-%H%M%S).log"

mkdir -p "$LOG_DIR"

on_error() {
  echo
  echo "[ERRO] Falha na linha $1."
  echo "[ERRO] Log salvo em: $RUN_LOG"
  echo
  read -p "Pressione ENTER para manter o terminal aberto..." _
}

on_exit() {
  echo
  echo "[FINALIZADO] Log salvo em: $RUN_LOG"
  echo
...
echo "[SUCESSO] Sprint 034 v3 instalada com sucesso."
