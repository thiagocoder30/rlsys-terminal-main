...
exports.RuntimeSanityEngine = void 0;
...
class RuntimeSanityEngine {
    evaluate(input, policy = {}) {
        ...
        if (input.dataIntegrityScore < activePolicy.minDataIntegrityScore) {
            ...
            return {
                ok: true,
                value: this.buildReport(input, 0, pressureScore, 'PARADIGM_BREAK', 'DATA_INTEGRITY_DEGRADED')
            };
        }
        if (input.snapshotConfidence < activePolicy.minSnapshotConfidence) {
            ...
            return {
                ok: true,
                value: this.buildReport(input, 0, pressureScore, 'SANITY_REVIEW', 'SNAPSHOT_CONFIDENCE_LOW')
            };
        }
        ...
        if (divergenceScore >= activePolicy.paradigmBreakDivergenceThreshold ||
            pressureScore >= activePolicy.paradigmBreakPressureThreshold) {
            return {
                ok: true,
                value: this.buildReport(input, divergenceScore, pressureScore, 'PARADIGM_BREAK', 'RUNTIME_DIVERGED_FROM_SNAPSHOT')
            };
        }
        if (divergenceScore >= activePolicy.reviewDivergenceThreshold ||
            input.regimeMismatchScore >= 0.35 ||
            input.spatialDriftScore >= 0.35) {
            return {
                ok: true,
                value: this.buildReport(input, divergenceScore, pressureScore, 'SANITY_REVIEW', 'RUNTIME_REVIEW_REQUIRED')
            };
        }
        return {
            ok: true,
            value: this.buildReport(input, divergenceScore, pressureScore, 'SANITY_OK', 'RUNTIME_MATCHES_SNAPSHOT')
        };
    }
    mergePolicy(policy) {
        return {
    ...
    computeDistributionDivergence(distribution) {
...
exports.RuntimeSanityEngine = RuntimeSanityEngine;
