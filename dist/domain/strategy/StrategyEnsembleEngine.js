...
exports.StrategyEnsembleEngine = void 0;
...
class StrategyEnsembleEngine {
    ...
    evaluate(votes) {
        try {
            if (!Array.isArray(votes))
                throw new Error('invalid_strategy_ensemble_votes');
            ...
            return (0, Result_1.err)(new Result_1.DomainError(message, 'STRATEGY_ENSEMBLE_FAILED'));
        }
    }
    getAccumulator(targets, vote) {
    ...
    decision(selectedTarget, blockers) {
        if (blockers.some(blocker => blocker.includes('bloqueada')))
            return 'BLOCKED';
        if (!selectedTarget)
            return 'INSUFFICIENT_SUPPORT';
        if (selectedTarget.conflictScore > this.options.maxConflictScore)
            return 'CONFLICT';
        if (blockers.length > 0)
            return 'INSUFFICIENT_SUPPORT';
        return 'CONSENSUS';
    }
    blockers(selectedTarget, blockedWeight) {
        const blockers = [];
        if (blockedWeight >= this.options.minSupportWeight)
            blockers.push('Peso bloqueado por estratégias vetadas excede limite do ensemble.');
        if (!selectedTarget) {
            blockers.push('Nenhum alvo recebeu votos direcionais suficientes para formar ensemble.');
            return blockers;
        }
        if (selectedTarget.supportVotes < this.options.minSupportVotes)
            blockers.push('Quantidade de votos de suporte abaixo do mínimo do ensemble.');
        if (selectedTarget.supportWeight < this.options.minSupportWeight)
            blockers.push('Peso de suporte abaixo do mínimo do ensemble.');
        if (selectedTarget.consensusScore < this.options.minConsensusScore)
            blockers.push('Score de consenso abaixo do limiar institucional.');
        if (selectedTarget.averageRiskPenalty > this.options.maxAverageRiskPenalty)
            blockers.push('Risco médio do alvo excede limite do ensemble.');
        if (selectedTarget.conflictScore > this.options.maxConflictScore)
            blockers.push('Conflito entre estratégias excede limite do ensemble.');
        return blockers;
    }
    warnings(selectedTarget, abstainWeight, targets) {
        const warnings = [];
        if (abstainWeight > 0)
            warnings.push('Parte das estratégias optou por abstenção, reduzindo convicção agregada.');
        if (targets.length > 1)
            warnings.push('Mais de um alvo recebeu votos; arbitragem por consenso foi aplicada.');
        if (selectedTarget && selectedTarget.opposeVotes > 0)
            warnings.push('Alvo selecionado possui oposição explícita de ao menos uma estratégia.');
        return warnings;
    }
    voteWeight(vote) {
    ...
    validateOptions(options) {
        if (!Number.isInteger(options.minSupportVotes) || options.minSupportVotes < 1)
            throw new Error('invalid_ensemble_min_support_votes');
        if (!isUnit(options.minConsensusScore))
            throw new Error('invalid_ensemble_min_consensus_score');
        if (!isUnit(options.maxConflictScore))
            throw new Error('invalid_ensemble_max_conflict_score');
        if (!isUnit(options.minSupportWeight))
            throw new Error('invalid_ensemble_min_support_weight');
        if (!isUnit(options.maxAverageRiskPenalty))
            throw new Error('invalid_ensemble_max_average_risk_penalty');
    }
    validateVote(vote) {
        if (!vote.strategyId.trim())
            throw new Error('invalid_ensemble_vote_strategy_id');
        if (!vote.label.trim())
            throw new Error('invalid_ensemble_vote_label');
        if (!vote.targetId.trim())
            throw new Error('invalid_ensemble_vote_target_id');
        if (!vote.targetLabel.trim())
            throw new Error('invalid_ensemble_vote_target_label');
        if (!isUnit(vote.confidence))
            throw new Error('invalid_ensemble_vote_confidence');
        if (!isUnit(vote.evidenceScore))
            throw new Error('invalid_ensemble_vote_evidence_score');
        if (!isUnit(vote.riskPenalty))
            throw new Error('invalid_ensemble_vote_risk_penalty');
        if (!isUnit(vote.recencyWeight))
            throw new Error('invalid_ensemble_vote_recency_weight');
        if (!isUnit(vote.weight))
            throw new Error('invalid_ensemble_vote_weight');
    }
}
exports.StrategyEnsembleEngine = StrategyEnsembleEngine;
