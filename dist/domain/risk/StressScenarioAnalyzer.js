...
exports.StressScenarioAnalyzer = void 0;
...
class StressScenarioAnalyzer {
    ...
    replayScenario(sectors, scenario) {
    ...
    pnl(target, won, scenario) {
        const stake = this.options.baseStakeFraction * scenario.stakeMultiplier;
        if (!won)
            return -stake * scenario.lossShockMultiplier;
        const sectorSize = RouletteStats_1.RouletteStats.SECTORS[target]?.length ?? 0;
    ...
    summarize(sampleSize, scenarios, surface) {
    ...
    blockers(summary, failedScenarios, watchedScenarios, surfaceFailRate) {
        const blockers = [];
        if (summary.sampleSize < 180)
            blockers.push('insufficient_stress_sample');
        if (summary.worstDrawdown > 0.45)
            blockers.push('catastrophic_drawdown_surface');
        if (summary.worstRuinProbabilityProxy > 0.45)
            blockers.push('ruin_probability_above_limit');
        if (failedScenarios > 0)
            blockers.push('stress_scenario_failed');
        if (surfaceFailRate > 0.35)
            blockers.push('drawdown_surface_failure_cluster');
        if (watchedScenarios >= 3)
            blockers.push('multiple_watch_scenarios');
        if (summary.resilienceScore < 0.45)
            blockers.push('low_resilience_score');
        return blockers;
    }
    approval(summary, blockers) {
        if (blockers.some(code => ['catastrophic_drawdown_surface', 'ruin_probability_above_limit', 'stress_scenario_failed', 'drawdown_surface_failure_cluster'].includes(code)))
            return 'REJECTED';
        if (summary.resilienceScore >= 0.68 && summary.tailRiskScore <= 0.28 && blockers.length === 0)
            return 'RESILIENT_CANDIDATE';
        return 'RESEARCH_REVIEW';
    }
    tailLoss(returns, percentile) {
    ...
    ruinProxy(maxDrawdown, roi, tailLoss95, longestUnderwaterRun, sampleSize) {
    ...
    grade(maxDrawdown, ruinProbabilityProxy, roi, tailLoss95) {
        if (maxDrawdown > 0.45 || ruinProbabilityProxy > 0.45 || tailLoss95 > this.options.baseStakeFraction * 4 || roi < -0.35)
            return 'FAIL';
        if (maxDrawdown > 0.25 || ruinProbabilityProxy > 0.25 || roi < 0)
            return 'WATCH';
        return 'PASS';
    }
}
exports.StressScenarioAnalyzer = StressScenarioAnalyzer;
