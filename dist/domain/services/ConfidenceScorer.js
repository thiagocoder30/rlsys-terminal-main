...
class ConfidenceScorer {
    score(analysis, backtest) {
        ...
        if (entropyScore < 0.35)
            reasons.push('Entropia próxima do aleatório; baixa confiança no padrão detectado.');
        if (signalScore < 0.55)
            reasons.push('Sinais estatísticos fracos ou inconsistentes.');
        if (sampleScore < 0.25)
            reasons.push('Amostra ainda pequena para uma decisão institucional.');
        if (!backtest)
            reasons.push('Sem backtest walk-forward suficiente para confirmar robustez fora da amostra.');
        const finalScore = this.clamp(entropyScore * 0.25 +
            signalScore * 0.25 +
            sampleScore * 0.20 +
            backtestScore * 0.30);
        return {
    ...
    backtestScore(backtest, reasons) {
        ...
        if (backtest.trades < 30)
            reasons.push('Menos de 30 trades walk-forward.');
        if (backtest.expectancyPerTrade <= 0)
            reasons.push('Expectativa walk-forward não positiva.');
        if (backtest.maxDrawdown > 0.2)
            reasons.push('Drawdown walk-forward acima do limite institucional.');
