...
exports.StatisticalSignificanceEngine = void 0;
...
class StatisticalSignificanceEngine {
    ...
    countValues(values) {
        ...
        values.forEach(value => {
            if (Number.isInteger(value) && value >= 0 && value < ROULETTE_SLOTS)
                counts[value] += 1;
        });
        return counts;
    }
    toDeviation(value, observed, expected, sampleSize) {
    ...
    evidenceScore(input) {
        ...
        const samplePenalty = input.sampleSize < 500 ? 0.72 : input.sampleSize < 2000 ? 0.88 : 1;
        return (pComponent * 0.35 + entropyComponent * 0.2 + divergenceComponent * 0.25 + zComponent * 0.2) * samplePenalty;
    }
    verdict(score, pValue, sampleSize) {
        if (sampleSize < 120)
            return 'NO_EVIDENCE';
        if (score >= 0.78 && pValue < 0.01)
            return 'STRONG_EVIDENCE';
        if (score >= 0.62 && pValue < 0.05)
            return 'MODERATE_EVIDENCE';
        if (score >= 0.45 || pValue < 0.1)
            return 'WEAK_EVIDENCE';
        return 'NO_EVIDENCE';
    }
    risk(verdict, sampleSize) {
        if (sampleSize < 500)
            return 'high';
        if (verdict === 'STRONG_EVIDENCE' || verdict === 'MODERATE_EVIDENCE')
            return 'moderate';
        return 'high';
    }
    recommendations(sampleSize, pValue, normalizedEntropy, verdict) {
        const recommendations = [];
        if (sampleSize < 500)
            recommendations.push('Aumentar amostra para pelo menos 500 spins antes de classificar significância operacional.');
        if (sampleSize < 10000)
            recommendations.push('Para pesquisa institucional, coletar 10.000+ spins por mesa/regime.');
        if (pValue < 0.05)
            recommendations.push('Validar o desvio em out-of-sample antes de liberar qualquer hipótese de edge.');
        if (normalizedEntropy < 0.92)
            recommendations.push('Investigar compressão de entropia com segmentação temporal e controle de duplicatas.');
        if (verdict === 'NO_EVIDENCE')
            recommendations.push('Não inferir edge: distribuição compatível com ruído ou amostra insuficiente.');
        return recommendations;
    }
}
exports.StatisticalSignificanceEngine = StatisticalSignificanceEngine;
...
function gammaRegularizedQ(a, x) {
    if (x < 0 || a <= 0)
        return Number.NaN;
    if (x === 0)
        return 1;
    if (x < a + 1)
        return 1 - gammaRegularizedPSeries(a, x);
    return gammaRegularizedQContinuedFraction(a, x);
}
function gammaRegularizedPSeries(a, x) {
    let sum = 1 / a;
    let del = sum;
    let ap = a;
...
function gammaRegularizedQContinuedFraction(a, x) {
    const fpmin = 1e-30;
    let b = x + 1 - a;
    let c = 1 / fpmin;
