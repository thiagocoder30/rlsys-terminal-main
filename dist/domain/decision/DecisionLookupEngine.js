...
exports.DecisionLookupEngine = void 0;
const DecisionContracts_1 = require("./DecisionContracts");
class DecisionLookupEngine {
    /**
     ...
     */
    static evaluate(liveState, snapshot) {
        ...
        if (!activeEdges || activeEdges.length === 0) {
            return this.buildResult(DecisionContracts_1.ActionSignal.NO_GO, 0, 0, 'REGIME_NOT_IN_SNAPSHOT');
        }
        ...
        if (!bestEdge) {
            return this.buildResult(DecisionContracts_1.ActionSignal.NO_GO, 0, 0, 'SECTOR_NO_EDGE');
        }
        ...
        if (bestEdge.confidenceScore < 0.85) {
            return this.buildResult(DecisionContracts_1.ActionSignal.OBSERVE, bestEdge.expectedEV, bestEdge.confidenceScore, 'WEAK_CONFIDENCE');
        }
        // 6. Autorização Final de Ação
        return this.buildResult(DecisionContracts_1.ActionSignal.SIGNAL, bestEdge.expectedEV, bestEdge.confidenceScore, 'ALPHA_CONFIRMED');
    }
    static buildResult(action, expectedEV, confidence, reason) {
        return { action, expectedEV, confidence, reason };
    }
}
exports.DecisionLookupEngine = DecisionLookupEngine;
