...
exports.MonteCarloV2Service = void 0;
...
const MonteCarloV2Engine_1 = require("../../domain/simulation/MonteCarloV2Engine");
class MonteCarloV2Service {
    constructor() {
    ...
    evaluate(input) {
        ...
        const engine = new MonteCarloV2Engine_1.MonteCarloV2Engine({ simulations: values.length >= 600 ? 320 : 180, blockSize: Math.max(8, Math.floor(Math.sqrt(values.length))) });
    ...
    headline(status, simulation) {
        if (status === 'ROBUSTNESS_CANDIDATE')
            return 'Hipótese candidata à robustez sob bootstrap, ainda sem liberação operacional.';
        if (status === 'REJECTED')
            return 'Simulação probabilística rejeitou a hipótese sob risco, cauda ou fragilidade.';
    ...
    recommendations(simulation) {
        ...
        if (simulation.summary.ruinProbability > 0.18)
            recommendations.push('Reduzir sizing e revisar hipótese: probabilidade de ruína acima do patamar institucional.');
        if (simulation.summary.p95MaxDrawdown > 0.35)
            recommendations.push('Executar stress adicional: drawdown P95 elevado nas simulações reamostradas.');
        if (simulation.summary.bootstrapConsistency < 0.55)
            recommendations.push('Investigar dependência da amostra original: consistência bootstrap insuficiente.');
        if (simulation.summary.sequenceDependencyRisk > 0.55)
            recommendations.push('Aplicar análise de dependência sequencial antes de qualquer hipótese operacional.');
        if (simulation.summary.tailRisk === 'HIGH' || simulation.summary.tailRisk === 'CRITICAL')
            recommendations.push('Submeter a revisão manual de risco de cauda e cenários extremos.');
        if (simulation.governance.reviewStatus === 'ROBUSTNESS_CANDIDATE')
            recommendations.push('Executar validação adversarial, bootstrap com blocos alternativos e análise por regime.');
        return [...new Set(recommendations)];
    }
}
exports.MonteCarloV2Service = MonteCarloV2Service;
